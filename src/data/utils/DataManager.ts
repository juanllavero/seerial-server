...
export class DataManager {
  ...
  public static async scanTVShow(folder: string, wsManager: WebSocketManager) {
    ...
    if (!exists) {
      // Add show to view
      Utils.addSeries(wsManager, this.library.id, show);
    }
    ...
    // Update show in view
    Utils.updateSeries(wsManager, this.library.id, show);
  }
  ...
  private static async processEpisode(
    ...
    if (!season) {
      ...
      // Add season to view
      Utils.addSeason(wsManager, this.library.id, season);
    }
    ...
    if (episode) {
      ...
      // Add episode to view
      Utils.addEpisode(wsManager, this.library.id, season.seriesID, episode);
    }
  }
  ...
  private static async setSeriesMetadataAndImages(
    ...
    if (credits && credits.crew) {
      if (show.creator.length > 0) show.creator.splice(0, show.creator.length);
  ...
  public static async scanMusic(folder: string, wsManager: WebSocketManager) {
    ...
    if (!this.library.getAnalyzedFolders().get(folder)) {
      ...
      // Add show to view
      Utils.addSeries(wsManager, this.library.id, collection);
    } else {
    ...
    // Update show in view
    Utils.updateSeries(wsManager, this.library.id, collection);
  }
  ...
  private static async processMusicFile(
    ...
    try {
      ...
      if (album === null) {
        ...
        // Add season to view
        Utils.addSeason(wsManager, this.library.id, album);
      }
      ...
      // Add episode to view
      Utils.addEpisode(wsManager, this.library.id, collection.id, song);
    } catch (error) {
