import {
  ...
  addArtistToAlbum,
  getAlbums,
} from "@/api/v0/albums/albums.service";
...
import { Album, Collection, Library } from "@/api/v0/index.models";
import { addSong } from "@/api/v0/songs/songs.service";
...
export async function scanMusic(library: Library, folder: string) {
  ...
  const collection = await addCollection({
    title: FilesManager.getFileName(folder),
  });
  ...
  const musicFiles = await FilesManager.getMusicFiles(folder);
...
export async function processMusicFile(
  ...
  try {
    ...
    const song = await addSong({
      title: title !== "" ? title : FilesManager.getFileName(musicFile),
      albumId: newAlbum.id,
      trackNumber,
      discNumber,
      composers,
    ...
    if (newAlbum.coverSrc === "") {
      ...
      let imageSrc = await FilesManager.findImageInFolder(
      ...
      if (!imageSrc) {
        ...
        imageSrc = await FilesManager.findImageInFolder(parentFolder);
      }
