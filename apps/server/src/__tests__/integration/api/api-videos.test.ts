...
beforeEach(async () => {
  ...
  mockContainer.useCases.getVideoById.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockVideo),
  });
  mockContainer.useCases.getVideoByEpisodeId.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockVideo),
  });
  mockContainer.useCases.getVideoPlaybackInfo.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockPlaybackInfo),
  });
  mockContainer.useCases.updateMediaInfo.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockVideo),
  });
  mockContainer.useCases.updateVideo.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockVideo),
  });
...
describe('Videos API', () => {
  ...
  describe('PATCH /api/videos/:id (adminAuth)', () => {
    ...
    it('returns 401 without auth', async () => {
      const res = await request(app).patch('/api/videos/video-1').send({ title: 'Updated Video' });
