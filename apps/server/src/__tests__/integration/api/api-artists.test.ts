...
beforeEach(async () => {
    ...
    mockContainer.useCases.getArtistById.mockReturnValue({ execute: jest.fn().mockResolvedValue(mockArtist) });
    mockContainer.useCases.updateArtist.mockReturnValue({ execute: jest.fn().mockResolvedValue({ ...mockArtist, name: 'Updated Artist' }) });
});
...
describe('Artists API', () => {
    ...
    it('GET /api/artists/:id returns 404 when artist not found', async () => {
        mockContainer.useCases.getArtistById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
