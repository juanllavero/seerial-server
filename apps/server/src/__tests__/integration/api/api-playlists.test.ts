...
beforeEach(async () => {
  ...
  mockContainer.useCases.getPlayLists.mockReturnValue({
    execute: jest.fn().mockResolvedValue([mockPlayList]),
  });
  mockContainer.useCases.getPlayListById.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockPlayList),
  });
  mockContainer.useCases.createPlayList.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockPlayList),
  });
  mockContainer.useCases.updatePlayList.mockReturnValue({
    execute: jest.fn().mockResolvedValue({ ...mockPlayList, title: 'Updated' }),
  });
  mockContainer.useCases.deletePlayList.mockReturnValue({
    execute: jest.fn().mockResolvedValue(undefined),
  });
  mockContainer.useCases.addSongToPlayList.mockReturnValue({
    execute: jest.fn().mockResolvedValue(undefined),
  });
  mockContainer.useCases.removeSongFromPlayList.mockReturnValue({
    execute: jest.fn().mockResolvedValue(undefined),
  });
});
