...
beforeEach(async () => {
    ...
    mockContainer.useCases.reorderLibraryItems.mockReturnValue(new ReorderLibraryItemsUseCase(libRepo));
    // deleteLibrary and scanLibrary are mocked (complex dependencies)
    mockContainer.useCases.deleteLibrary.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
...
describe('Libraries API', () => {
    ...
    describe('GET /api/libraries/:id', () => {
        it('returns 200 with library data for existing id', async () => {
            const lib = await libRepo.create(buildLibraryData({ name: 'My Movies', type: LibraryTypes.MOVIES }));
    ...
    describe('GET /api/libraries/:id/content', () => {
        it('returns 200 with empty content for a library with no items', async () => {
            const lib = await libRepo.create(buildLibraryData({ name: 'Empty', type: LibraryTypes.MOVIES }));
    ...
    describe('PATCH /api/libraries/:id', () => {
        it('returns 200 and updates library name', async () => {
            const lib = await libRepo.create(buildLibraryData({ name: 'Old Name', type: LibraryTypes.MOVIES }));
        ...
        it('returns 401 without auth', async () => {
            const lib = await libRepo.create(buildLibraryData({ name: 'A Library', type: LibraryTypes.MOVIES }));
    ...
    describe('DELETE /api/libraries/:id', () => {
        it('returns 200 when deleting a library (mocked use case)', async () => {
            const lib = await libRepo.create(buildLibraryData({ name: 'To Delete', type: LibraryTypes.MOVIES }));
    ...
    describe('POST /api/libraries/order', () => {
        it('returns 200 when reordering libraries', async () => {
            const lib1 = await libRepo.create(buildLibraryData({ name: 'First', type: LibraryTypes.MOVIES }));
            const lib2 = await libRepo.create(buildLibraryData({ name: 'Second', type: LibraryTypes.SHOWS }));
