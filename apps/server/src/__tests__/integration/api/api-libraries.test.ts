...
beforeEach(async () => {
	...
	mockContainer.useCases.getLibraries.mockReturnValue(
		new GetLibrariesUseCase(libRepo),
	);
	mockContainer.useCases.getLibrary.mockReturnValue(
		new GetLibraryUseCase(libRepo),
	);
	mockContainer.useCases.getLibraryContent.mockReturnValue(
		new GetLibraryContentUseCase(libRepo),
	);
	mockContainer.useCases.updateLibrary.mockReturnValue(
		new UpdateLibraryUseCase(libRepo),
	);
	mockContainer.useCases.reorderLibraries.mockReturnValue(
		new ReorderLibrariesUseCase(libRepo),
	);
...
describe("Libraries API", () => {
	describe("GET /api/libraries", () => {
		...
		it("returns 200 with all libraries", async () => {
			await libRepo.create(
				buildLibraryData({ name: "Movies", type: LibraryTypes.MOVIES }),
			);
			await libRepo.create(
				buildLibraryData({ name: "Shows", type: LibraryTypes.SHOWS }),
			);
