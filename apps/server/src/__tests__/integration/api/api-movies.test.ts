...
beforeEach(async () => {
    ...
    mockContainer.useCases.getMoviebyId.mockReturnValue({ execute: jest.fn().mockResolvedValue(mockMovie) });
    mockContainer.useCases.updateMovie.mockReturnValue({ execute: jest.fn().mockResolvedValue({ ...mockMovie, title: 'Updated Movie' }) });
    mockContainer.useCases.deleteMovie.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    mockContainer.useCases.refreshMovieMetadata.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
});
