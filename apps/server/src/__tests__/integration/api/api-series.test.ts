...
beforeEach(async () => {
    ...
    mockContainer.useCases.getSeriesById.mockReturnValue({ execute: jest.fn().mockResolvedValue(mockSeries) });
    mockContainer.useCases.updateSeries.mockReturnValue({ execute: jest.fn().mockResolvedValue({ ...mockSeries, name: 'Updated Show' }) });
    mockContainer.useCases.deleteSeries.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    mockContainer.useCases.refreshSeriesMetadata.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
});
...
describe('Series API', () => {
    describe('GET /api/series/:id (cookieAuth)', () => {
        ...
        it('returns 404 when series not found', async () => {
            mockContainer.useCases.getSeriesById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
