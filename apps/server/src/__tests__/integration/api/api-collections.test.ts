...
beforeEach(async () => {
    ...
    mockContainer.useCases.getMusicExtras.mockReturnValue({ execute: jest.fn().mockResolvedValue([]) });
    mockContainer.useCases.reorderCollectionItems.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    mockContainer.useCases.getCollectionById.mockReturnValue({ execute: jest.fn().mockResolvedValue(mockCollection) });
    mockContainer.useCases.updateCollection.mockReturnValue({ execute: jest.fn().mockResolvedValue({ ...mockCollection, title: 'Updated' }) });
    mockContainer.useCases.deleteCollection.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
});
