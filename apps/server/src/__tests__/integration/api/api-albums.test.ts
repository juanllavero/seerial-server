...
beforeEach(async () => {
  ...
  mockContainer.useCases.getAlbumById.mockReturnValue({
    execute: jest.fn().mockResolvedValue(mockAlbum),
  });
  mockContainer.useCases.updateAlbum.mockReturnValue({
    execute: jest.fn().mockResolvedValue({ ...mockAlbum, title: 'Updated Album' }),
  });
  mockContainer.useCases.deleteAlbum.mockReturnValue({
