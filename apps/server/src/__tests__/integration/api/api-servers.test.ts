...
jest.mock('@/api/v1/servers/infrastructure/services/ServerConfigService', () => ({
  ServerConfigService: {
    serverConfig: {
      id: 'test-server-id',
      name: 'Test Server',
...
beforeEach(async () => {
  ...
  mockContainer.useCases.getAllUsers.mockReturnValue(new GetAllUsersUseCase(usersRepo));
...
describe('Servers API', () => {
  ...
  describe('PATCH /api/servers/:id', () => {
    ...
    it('returns 401 without auth token', async () => {
      const res = await request(app).patch('/api/servers/test-server-id').send({ name: 'X' });
