...
beforeEach(async () => {
    ...
    mockContainer.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue(mockSeason) });
    mockContainer.useCases.updateSeason.mockReturnValue({ execute: jest.fn().mockResolvedValue({ ...mockSeason, name: 'Updated Season' }) });
    mockContainer.useCases.deleteSeason.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    mockContainer.useCases.setEpisodeWatchState.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
});
