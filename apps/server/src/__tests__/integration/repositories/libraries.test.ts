...
describe("LibrariesRepositoryImpl", () => {
	...
	describe("getContent", () => {
		it("returns music collections and standalone albums without relying on a full library graph query", async () => {
			...
			expect(content.map((item) => item.id)).toEqual([
				collection.id,
				standaloneAlbum.id,
			]);
	...
	describe("addAnalyzedFile / removeAnalyzedFile", () => {
		it("adds and removes an analyzed file", async () => {
			...
			const after = (await repo.getById(lib!.id)) as unknown as Record<
				string,
				unknown
			>;
			const analyzedFilesAfterAdd = after.analyzedFiles as Record<
				string,
				string
			>;
			...
			const afterRemove = (await repo.getById(lib!.id)) as unknown as Record<
				string,
				unknown
			>;
			const analyzedFilesAfterRemove = afterRemove.analyzedFiles as Record<
				string,
				string
			>;
