...
describe('LibrariesRepositoryImpl', () => {
    describe('create', () => {
        ...
        it('creates libraries of different types', async () => {
            const movies = await repo.create(buildLibraryData({ type: LibraryTypes.MOVIES, name: 'Movies' }));
            const shows = await repo.create(buildLibraryData({ type: LibraryTypes.SHOWS, name: 'Shows' }));
            const music = await repo.create(buildLibraryData({ type: LibraryTypes.MUSIC, name: 'Music' }));
    ...
    describe('update', () => {
        ...
        it('throws when data is null/undefined', async () => {
            const lib = await repo.create(buildLibraryData());
            await expect(repo.update(lib!.id, null as never)).rejects.toMatchObject({ statusCode: 400 });
        });
    });
    ...
    describe('addAnalyzedFile / removeAnalyzedFile', () => {
        ...
        it('throws NotFoundException for non-existent library', async () => {
            await expect(
                repo.addAnalyzedFile('bad-id', '/path/file.mkv', 'v1'),
