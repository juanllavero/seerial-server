...
describe('EpisodeRepositoryImpl', () => {
  describe('create / findById', () => {
    ...
    it('findById returns the episode for a valid id', async () => {
      const season = await createSeason();
      const created = await repo.create({ seasonId: season.id, name: 'Episode 2' });
    ...
    it('returns the existing episode when created with the same id twice', async () => {
      ...
      const second = await repo.create({ id: first!.id, seasonId: season.id, name: 'Ep 1 Dup' });
  ...
  describe('update', () => {
    it('updates the episode name', async () => {
      const season = await createSeason();
      const episode = await repo.create({ seasonId: season.id, name: 'Old Name' });
  ...
  describe('delete', () => {
    it('deletes the episode so it can no longer be found', async () => {
      const season = await createSeason();
      const episode = await repo.create({ seasonId: season.id, name: 'To Delete' });
