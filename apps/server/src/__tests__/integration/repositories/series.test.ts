...
describe('SeriesRepositoryImpl', () => {
    describe('create / findById', () => {
        it('creates a series with a generated id', async () => {
            ...
            const series = await repo.create({ libraryId: library.id, name: 'Breaking Bad' });
        ...
        it('findById returns the series for a valid id', async () => {
            const library = await createLibrary();
            const created = await repo.create({ libraryId: library.id, name: 'The Wire' });
    ...
    describe('update', () => {
        it('updates series name', async () => {
            const library = await createLibrary();
            const series = await repo.create({ libraryId: library.id, name: 'Old Title' });
    ...
    describe('delete', () => {
        it('deletes the series so it can no longer be found', async () => {
            const library = await createLibrary();
            const series = await repo.create({ libraryId: library.id, name: 'To Delete' });
