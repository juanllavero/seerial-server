...
jest.mock('@/api/v1/shared/infrastructure/adapters/di/container', () => ({
    useCases: {
        ...
        getLibrary: jest.fn().mockReturnValue({ execute: jest.fn().mockResolvedValue(null) }),
        getEpisodeById: jest.fn().mockReturnValue({ execute: jest.fn().mockResolvedValue(null) }),
        getSeasonById: jest.fn().mockReturnValue({ execute: jest.fn().mockResolvedValue(null) }),
        getSeriesById: jest.fn().mockReturnValue({ execute: jest.fn().mockResolvedValue(null) }),
...
const ffmpegMediaInfo = jest.requireMock('@/api/v1/shared/infrastructure/adapters/ffmpeg/mediaInfo');
...
describe('VideosRepositoryImpl', () => {
    ...
    describe('getVideoPlaybackInfo', () => {
        it('throws when target video is missing', async () => {
            await expect(repo.getVideoPlaybackInfo('missing-video')).rejects.toMatchObject({ statusCode: 404 });
        });
        ...
        it('throws when movie cannot be resolved', async () => {
            ...
            container.useCases.getMoviebyId.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });

            await expect(repo.getVideoPlaybackInfo(video.id)).rejects.toMatchObject({ statusCode: 404 });
        });
        ...
        it('throws when library cannot be resolved for movie context', async () => {
            ...
            container.useCases.getLibrary.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('returns playback info for a movie video when dependencies are available', async () => {
            ...
            ffmpegMediaInfo.getMediaInfo.mockResolvedValue({ format: { duration: 100 } });
        ...
        it('throws when media info is missing', async () => {
            ...
            container.useCases.getLibrary.mockReturnValue({ execute: jest.fn().mockResolvedValue({}) });
        ...
        it('returns playback info for an episode video', async () => {
            ...
            container.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue(episode) });
            container.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue(season) });
            container.useCases.getSeriesById.mockReturnValue({ execute: jest.fn().mockResolvedValue(series) });
            ffmpegMediaInfo.getMediaInfo.mockResolvedValue({ format: { duration: 50 } });
        ...
        it('throws when episode cannot be resolved in episode context', async () => {
            ...
            container.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('throws when season cannot be resolved in episode context', async () => {
            ...
            container.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue(episode) });
            container.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('throws when series cannot be resolved in episode context', async () => {
            ...
            container.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue(episode) });
            container.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue(season) });
            container.useCases.getSeriesById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
