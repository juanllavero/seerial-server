...
describe('CollectionsRepositoryImpl', () => {
    ...
    it('adds and removes movie/series/album relationships', async () => {
        ...
        expect(await CollectionMovieModel.findOne({ where: { collectionId: collection!.id, movieId: movie.id } })).not.toBeNull();
        expect(await CollectionSeriesModel.findOne({ where: { collectionId: collection!.id, seriesId: series.id } })).not.toBeNull();
        expect(await CollectionAlbumModel.findOne({ where: { collectionId: collection!.id, albumId: album.id } })).not.toBeNull();
        ...
        expect(await CollectionMovieModel.findOne({ where: { collectionId: collection!.id, movieId: movie.id } })).toBeNull();
        expect(await CollectionSeriesModel.findOne({ where: { collectionId: collection!.id, seriesId: series.id } })).toBeNull();
        expect(await CollectionAlbumModel.findOne({ where: { collectionId: collection!.id, albumId: album.id } })).toBeNull();
    });
    ...
    it('reorders collection content across media types', async () => {
        ...
        const orderedSeries = await CollectionSeriesModel.findOne({ where: { collectionId: collection!.id, seriesId: series.id } });
        const orderedMovie = await CollectionMovieModel.findOne({ where: { collectionId: collection!.id, movieId: movie.id } });
