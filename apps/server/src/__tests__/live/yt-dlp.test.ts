...
jest.mock("@/api/v1/shared/infrastructure/adapters/di/container", () => ({
	fileSystemService: {
		getExternalPath: jest.fn((targetPath: string) =>
			path.join(liveBinRoot, targetPath),
		),
...
const { DownloaderServiceImpl } =
	require("@/api/v1/shared/infrastructure/adapters/downloader/DownloaderServiceImpl") as {
...
describeIfLive("DownloaderServiceImpl – Live yt-dlp (Phase 3)", () => {
	...
	it("returns search results for a known query", async () => {
		...
		if (results.length === 0) {
			console.warn(
				"yt-dlp search returned no results; skipping strict assertions",
			);
			return;
		}
