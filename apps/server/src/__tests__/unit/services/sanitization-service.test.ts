...
describe("SanitizationService", () => {
	...
	it("builds POSIX-compatible candidates for Windows-formatted paths on unix-like systems", () => {
		...
		expect(candidates).toEqual(
			expect.arrayContaining([
				"/mnt/f/media/movie.mkv",
				"/Volumes/f/media/movie.mkv",
			]),
		);
	});
	...
	it("enforces allowed base paths", () => {
		...
		expect(() =>
			sanitizeFilePath(target, [path.join(tmpDir, "other")]),
	...
	it("validates extension and existing file when required", () => {
		...
		expect(
			sanitizeFilePathWithExtension(good, [".mp4"], [tmpDir], true),
		).toBeTruthy();
		expect(() =>
			sanitizeFilePathWithExtension(good, [".mkv"], [tmpDir], false),
	...
	it("rejects missing files and directory values when a file is required", () => {
		...
		expect(() =>
			sanitizeFilePathWithExtension(missing, [".mp4"], [tmpDir], true),
		).toThrow("File does not exist");
		expect(() =>
			sanitizeFilePathWithExtension(folder, [".mp4"], [tmpDir], true),
	...
	it("sanitizes multiple file paths and validates argument type", () => {
		...
		expect(() => sanitizeMultipleFilePaths({} as unknown as string[])).toThrow(
			"must be an array",
		);
	});
	...
	it("safely joins path and rejects invalid file names", () => {
		...
		expect(() => safeJoinPath(tmpDir, "../bad.jpg")).toThrow(
	...
	it("sanitizes directories and rejects missing or non-directory paths", () => {
		...
		expect(() =>
			sanitizeDirectoryPath(path.join(tmpDir, "missing"), [tmpDir], true),
		).toThrow("Directory does not exist");
		expect(() => sanitizeDirectoryPath(filePath, [tmpDir], true)).toThrow(
	...
	it("includes root and mounted volumes for unix-like systems", () => {
		...
		jest
			...
			.mockReturnValue(["Media", "Archive"] as unknown as fs.Dirent[]);
	...
	it("ignores volume read failures on unix-like systems", () => {
		...
		jest
			.spyOn(fs, "existsSync")
			.mockImplementation((target: fs.PathLike) => target === "/Volumes");
