...
describe('SanitizationService', () => {
  ...
  it('builds POSIX-compatible candidates for Windows-formatted paths on unix-like systems', () => {
    ...
    expect(candidates).toEqual(
      expect.arrayContaining(['/mnt/f/media/movie.mkv', '/Volumes/f/media/movie.mkv']),
    );
  });
  ...
  it('enforces allowed base paths', () => {
    ...
    expect(() => sanitizeFilePath(target, [path.join(tmpDir, 'other')])).toThrow(
  ...
  it('validates extension and existing file when required', () => {
    ...
    expect(() => sanitizeFilePathWithExtension(good, ['.mkv'], [tmpDir], false)).toThrow(
  ...
  it('rejects missing files and directory values when a file is required', () => {
    ...
    expect(() => sanitizeFilePathWithExtension(missing, ['.mp4'], [tmpDir], true)).toThrow(
      'File does not exist',
    );
    expect(() => sanitizeFilePathWithExtension(folder, ['.mp4'], [tmpDir], true)).toThrow(
      'Path is not a file',
    );
  });
  ...
  it('sanitizes directories and rejects missing or non-directory paths', () => {
    ...
    expect(() => sanitizeDirectoryPath(path.join(tmpDir, 'missing'), [tmpDir], true)).toThrow(
