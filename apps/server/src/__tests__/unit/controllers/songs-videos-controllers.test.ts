...
const { findLyricsForSong } = jest.requireMock('@/api/v1/shared/infrastructure/services/MediaDetailsService');
const { verifyAudioStreamToken } = jest.requireMock('@/middleware/audio.middleware');
...
describe('SongsController', () => {
    ...
    it('updates a song', async () => {
        ...
        const response = await new SongsController().update('song-1', { title: 'Updated' } as never);
    ...
    it('throws not found when song does not exist while adding lyrics', async () => {
        ...
        await expect(
            new SongsController().addSongsLyrics({ songId: 'missing', language: 'en', content: '[00:00.00] Hi' }),
    ...
    it('adds song lyrics with language suffix for non-original language', async () => {
        ...
        expect(container.fileSystemService.writeFile).toHaveBeenCalledWith('/music/song.en.lrc', '[00:00.00] Hello', 'utf-8');
    });
    ...
    it('adds song lyrics without language suffix when language is "original"', async () => {
        ...
        expect(container.fileSystemService.join).toHaveBeenCalledWith('/music', 'song.lrc');
    });

    it('generates a signed song stream URL', async () => {
        const response = await new SongsController().getSongUrl(
            { filePath: '/music/song.mp3', expiresIn: '5m' },
        );
    ...
    it('propagates middleware errors in streamAudio', async () => {
        (verifyAudioStreamToken as jest.Mock).mockImplementation((_req, _res, next) => next(new Error('Invalid token')));

        const req = { res: {}, audioParams: { path: '/song.mp3' } } as never;

        await expect(new SongsController().streamAudio(req)).rejects.toThrow('Invalid token');
    });

    it('streams audio when token is valid', async () => {
        (verifyAudioStreamToken as jest.Mock).mockImplementation((_req, _res, next) => next());
        container.audioProcessingService.getStreamableAudioPath.mockResolvedValue('/processed/song.mp3');
        ...
        expect(container.audioProcessingService.getStreamableAudioPath).toHaveBeenCalledWith('/song.mp3', false);
        expect(container.audioProcessingService.streamFile).toHaveBeenCalledWith('/processed/song.mp3', req, {});
    });
});
...
describe('VideosController', () => {
    ...
    it('throws not found when video does not exist in get', async () => {
        ...
        await expect(new VideosController().get('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('throws not found when video by episode id does not exist', async () => {
        ...
        await expect(new VideosController().getByEpisodeId('ep-missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('throws not found when playback info does not exist', async () => {
        ...
        await expect(new VideosController().getPlaybackInfo('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('updates a video', async () => {
        ...
        const response = await new VideosController().update('vid-1', { title: 'Updated' } as never);
    ...
    it('throws 400 when userId is missing in setWatchState', async () => {
        await expect(
            new VideosController().setWatchState('vid-1', { watched: true } as never, asRequest({})),
    ...
    it('adds video to watch list when watched=true', async () => {
        ...
        expect(container.useCases.addVideoToWatchList().execute).toHaveBeenCalledWith('vid-1', 'user-1');
    });
    ...
    it('removes video from watch list when watched=false', async () => {
        ...
        expect(container.useCases.removeVideoFromWatchList().execute).toHaveBeenCalledWith('vid-1', 'user-2');
    });
});
