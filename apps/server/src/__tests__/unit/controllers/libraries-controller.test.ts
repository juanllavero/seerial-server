...
describe('LibrariesController', () => {
    ...
    it('throws not found when library does not exist in getById', async () => {
        ...
        await expect(new LibrariesController().getById('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('throws not found when library content is null', async () => {
        ...
        await expect(new LibrariesController().getContent('missing', asRequest({}))).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('throws not found when library does not exist in startScan', async () => {
        ...
        await expect(new LibrariesController().startScan('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('reorders libraries', async () => {
        ...
        const response = await new LibrariesController().reorder({ orderedLibraryIds: ['lib-1', 'lib-2'] });
    ...
    it('reorders library items', async () => {
        ...
        const response = await new LibrariesController().reorderItems('lib-1', { orderedItems: ['item-1', 'item-2'] } as never);
