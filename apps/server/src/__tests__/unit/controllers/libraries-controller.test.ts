...
describe("LibrariesController", () => {
	...
	it("throws not found when library does not exist in getById", async () => {
		...
		await expect(
			new LibrariesController().getById("missing"),
	...
	it("returns library content for authenticated user", async () => {
		...
		const response = await new LibrariesController().getContent(
			"lib-1",
			asRequest({}),
		);
	...
	it("throws not found when library does not exist in startScan", async () => {
		...
		await expect(
			new LibrariesController().startScan("missing"),
		).rejects.toMatchObject({
			statusCode: 404,
		});
	});
	...
	it("throws not found when scan returns null during create", async () => {
		...
		await expect(new LibrariesController().create(body)).rejects.toMatchObject({
			statusCode: 404,
		});
	});
	...
	it("updates a library", async () => {
		...
		const response = await new LibrariesController().update("lib-1", {
			name: "Updated",
		} as never);
