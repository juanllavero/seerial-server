...
const createUseCase = (result?: unknown) => ({
...
describe("MoviesController", () => {
	...
	it("delegates refresh, identification change, update and delete operations", async () => {
		...
		const updateExecute = jest
			.fn()
			.mockResolvedValue({ id: "movie-1", name: "New Name" });
		const deleteExecute = jest.fn().mockResolvedValue(undefined);

		container.useCases.refreshMovieMetadata.mockReturnValue({
			execute: refreshExecute,
		});
		container.useCases.updateMovieId.mockReturnValue({
			execute: updateIdExecute,
		});
		...
		await controller.changeIdentification("movie-1", {
			themdbId: 100,
		} as never);
		const updateResponse = await controller.update("movie-1", {
			name: "New Name",
		} as never);
	...
	it("throws not found when movie does not exist in get and setWatchState", async () => {
		...
		await expect(controller.get("movie-404")).rejects.toMatchObject({
			statusCode: 404,
		});
		await expect(
	...
	it("marks a movie as watched and removes all videos from continue watching", async () => {
		...
		container.useCases.addMovieToWatchList.mockReturnValue({
			execute: addMovieExecute,
		});
	...
	it("marks a movie as not watched and re-adds partially watched videos to continue watching", async () => {
		...
		container.useCases.getMoviebyId.mockReturnValue(createUseCase(movie));
		container.useCases.removeMovieFromWatchList.mockReturnValue({
			execute: removeMovieExecute,
		});
		container.useCases.addVideoToContinueWatching.mockReturnValue({
			execute: addContinueExecute,
		});
	...
	it("returns movie data, search results, imdb score and remaining videos", async () => {
		...
		const searchResponse = await controller.searchMovies(
			"Interstellar",
			"2014",
		);
		...
		expect(MediaService.countRemainingVideos).toHaveBeenCalledWith(
			"movie-1",
			"user-1",
		);
