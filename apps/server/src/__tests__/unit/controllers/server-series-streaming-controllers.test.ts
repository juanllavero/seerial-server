...
describe("Servers, Series and Video Streaming controllers", () => {
	...
	describe("SeriesController", () => {
		...
		it("marks nested episodes as watched and updates season and series watch lists", async () => {
			...
			const getEpisodeByIdExecute = jest
				.fn()
				.mockResolvedValue({ id: "episode-1" });
			const getVideoByEpisodeIdExecute = jest
				.fn()
				.mockResolvedValue({ id: "video-1" });
			...
			container.useCases.getSeriesById.mockReturnValue({
				execute: getSeriesByIdExecute,
			});
			container.useCases.getSeasonById.mockReturnValue({
				execute: getSeasonByIdExecute,
			});
			container.useCases.getEpisodeById.mockReturnValue({
				execute: getEpisodeByIdExecute,
			});
			container.useCases.getVideoByEpisodeId.mockReturnValue({
				execute: getVideoByEpisodeIdExecute,
			});
			container.useCases.addVideoToWatchList.mockReturnValue({
				execute: addVideoToWatchListExecute,
			});
			container.useCases.removeVideoFromContinueWatching.mockReturnValue({
				execute: removeContinueExecute,
			});
			container.useCases.updateVideo.mockReturnValue({
				execute: updateVideoExecute,
			});
			container.useCases.addSeasonToWatchList.mockReturnValue({
				execute: addSeasonExecute,
			});
			container.useCases.updateSeason.mockReturnValue({
				execute: updateSeasonExecute,
			});
			container.useCases.addSeriesToWatchList.mockReturnValue({
				execute: addSeriesExecute,
			});
			container.useCases.updateSeries.mockReturnValue({
				execute: updateSeriesExecute,
			});
			...
			expect(addVideoToWatchListExecute).toHaveBeenCalledWith(
				"video-1",
				"user-1",
			);
			...
			expect(updateSeriesExecute).toHaveBeenCalledWith(
				"series-1",
		...
		it("removes watch-list relations when setting as not watched and tolerates missing seasons", async () => {
			...
			container.useCases.removeSeriesFromWatchList.mockReturnValue({
				execute: removeSeriesExecute,
			});
			...
			expect(
				container.useCases.removeSeasonFromWatchList,
		...
		it("returns remaining episodes count for authenticated user", async () => {
			...
			expect(MediaService.countRemainingEpisodes).toHaveBeenCalledWith(
				"series-1",
				"user-1",
			);
	...
	describe("ServersController", () => {
		it("returns invalid api key status when token is missing", async () => {
			...
			container.useCases.getAllUsers.mockReturnValue(
				useCase([{ id: "user-1" }]),
			);
		...
		it("reads a single config key and returns null when key is missing", async () => {
			...
			container.fileSystemService.readFileSync.mockReturnValue(
				JSON.stringify({ autoScan: true }),
			);

			const response = await new ServersController().getServerConfigKey(
				"unknownKey",
			);
	...
	describe("VideoStreamingController", () => {
		it("builds a signed transcoded URL with default values", async () => {
			...
			expect(response.data).toBe(
				"/video-streaming/transcoded?token=signed-token",
			);
		});
		...
		it("builds a passthrough URL honoring explicit expiration", async () => {
			...
			expect(response.data).toBe(
				"/video-streaming/passthrough?token=signed-passthrough",
			);
		});
		...
		it("resolves localId to the downloaded video file before signing passthrough URL", async () => {
			...
			expect(
				container.fileSystemService.getValidVideoFiles,
			).toHaveBeenCalledWith("/data/resources/videos/video-1");
			expect(jwt.sign).toHaveBeenCalledWith(
				{
					userId: "user-2",
					path: "/data/resources/videos/video-1/video-1.mkv",
				},
				"jwt-secret",
				{ expiresIn: "2m" },
			);
		...
		it("returns null when the local video folder does not exist", async () => {
			container.fileSystemService.join.mockReturnValue(
				"resources/videos/video-missing",
			);
			container.fileSystemService.getExternalPath.mockReturnValue(
		...
		it("returns null when the local video folder has no valid video file", async () => {
			container.fileSystemService.join.mockReturnValue(
				"resources/videos/video-empty",
			);
			container.fileSystemService.getExternalPath.mockReturnValue(
				"/data/resources/videos/video-empty",
			);
			container.fileSystemService.isFolder.mockResolvedValue(true);
			container.fileSystemService.getValidVideoFiles.mockResolvedValue([]);
		...
		it("throws when streaming response is unavailable", async () => {
			await expect(
				new VideoStreamingController().streamVideo(asRequest({})),
			).rejects.toThrow("Streaming response object is not available");
		});
		...
		it("streams transcoded video after middleware succeeds", async () => {
			...
			expect(
				container.videoProcessingService.transcodeAndStreamVideo,
			).toHaveBeenCalledWith({ path: "/tmp/file.mkv" }, req.res);
		});

		it("propagates middleware errors in passthrough streaming", async () => {
			(verifyVideoStreamToken as jest.Mock).mockImplementationOnce(
				(_req, _res, next) => next(new Error("invalid token")),
			);

			await expect(
				new VideoStreamingController().streamVideoFile(
					asRequest({ res: {} as never } as never),
				),
			).rejects.toThrow("invalid token");
			expect(
				container.videoProcessingService.streamDirectVideoFile,
			).not.toHaveBeenCalled();
		});
	});
});
