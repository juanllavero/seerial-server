...
jest.mock(
	"@/api/v1/servers/infrastructure/persistence/models/ServerModel",
	() => ({
		ServerModel: {
			find: jest.fn(),
		},
...
const { ServerModel } = jest.requireMock(
	"@/api/v1/servers/infrastructure/persistence/models/ServerModel",
);
const sanitizationService = jest.requireMock(
	"@/api/v1/shared/infrastructure/services/SanitizationService",
);
const mediaDetailsService = jest.requireMock(
	"@/api/v1/shared/infrastructure/services/MediaDetailsService",
);
...
describe("Shared controllers", () => {
	beforeEach(() => {
		...
		mediaDetailsService.findMediaBackground.mockResolvedValue({
			background: "ok",
		});
		container.externalSearchService.searchDownloadableMedia.mockResolvedValue([
			{ id: "result-1" },
		]);
	...
	describe("FilesController", () => {
		...
		it("returns sorted folder contents excluding hidden items", async () => {
			const tmpDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "files-controller-"),
			);
	...
	describe("APIKeyController", () => {
		it("returns error when api key is empty", async () => {
			const response = await new APIKeyController().configureApiKey({
				apiKey: "",
			});
		...
		it("initializes tmdb and returns valid status for api key", async () => {
			...
			expect(propertiesReader).toHaveBeenCalledWith(
				container.fileSystemService.propertiesFilePath,
			);
	...
	describe("MediaController", () => {
		it("returns media details and media background for valid types", async () => {
			...
			const background = await controller.getMediaBackground(
				"movie",
				"video",
				"movie-1",
			);
			...
			expect(mediaDetailsService.getDetails).toHaveBeenCalledWith(
				"movie",
				"movie-1",
			);
			expect(background.success).toBe(true);
			expect(mediaDetailsService.findMediaBackground).toHaveBeenCalledWith(
				"video",
				"movie",
				"movie-1",
			);
		...
		it("throws when itemType or mediaType are invalid", async () => {
			...
			await expect(
				controller.getMediaBackground("album", "video", "id-1"),
			).rejects.toMatchObject({
				statusCode: 400,
			});
			await expect(
				controller.getMediaBackground("movie", "book", "id-1"),
			).rejects.toThrow("Invalid mediaType. Must be 'video' or 'music'.");
		});
	});
	...
	describe("ContinueWatchingController", () => {
		it("returns continue-watching videos for authenticated user", async () => {
			...
			const response = await new ContinueWatchingController().getVideos(
				req as never,
			);
	...
	describe("SearchController", () => {
		it("returns search results from external search service", async () => {
			const response = await new SearchController().searchDownloadableMedia(
				"interstellar trailer",
			);

			expect(
				container.externalSearchService.searchDownloadableMedia,
			).toHaveBeenCalledWith("interstellar trailer");
			expect(response.success).toBe(true);
