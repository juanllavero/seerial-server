...
describe("PlayListController", () => {
	...
	it("throws not found when playlist does not exist", async () => {
		...
		await expect(
			new PlayListController().getById("missing"),
	...
	it("creates a new playlist", async () => {
		...
		const response = await new PlayListController().create({
			name: "New",
		} as never);
	...
	it("updates a playlist", async () => {
		...
		const response = await new PlayListController().update("pl-1", {
			name: "Updated",
		} as never);
	...
	it("adds a song to a playlist", async () => {
		...
		const response = await new PlayListController().addSong("pl-1", {
			songId: "song-1",
		} as never);

		expect(response.message).toBe(messages.success.create);
		expect(container.useCases.addSongToPlayList().execute).toHaveBeenCalledWith(
			"pl-1",
			"song-1",
		);
	});
	...
	it("removes a song from a playlist", async () => {
		...
		const response = await new PlayListController().removeSong(
			"pl-1",
			"song-1",
		);
		...
		expect(response.message).toBe(messages.success.delete);
		expect(
			container.useCases.removeSongFromPlayList().execute,
		).toHaveBeenCalledWith("pl-1", "song-1");
	});
});
...
describe("SeasonsController", () => {
	...
	it("throws not found when season does not exist", async () => {
		...
		await expect(new SeasonsController().get("missing")).rejects.toMatchObject({
			statusCode: 404,
		});
	});
	...
	it("sets episode watch state for the last episode when marking season as watched", async () => {
		...
		container.useCases.getSeasonById
			...
			.mockReturnValueOnce(uc(season));
		container.useCases.setEpisodeWatchState.mockReturnValue({
			execute: episodeExec,
		});
...
describe("EpisodesController", () => {
	...
	it("updates an episode", async () => {
		...
		const response = await new EpisodesController().update("ep-1", {
			title: "Pilot",
		} as never);
	...
	it("throws 400 when userId is missing in setWatchState", async () => {
		await expect(
			new EpisodesController().setWatchState(
				"ep-1",
				{ state: true } as never,
				asRequest({}),
			),
