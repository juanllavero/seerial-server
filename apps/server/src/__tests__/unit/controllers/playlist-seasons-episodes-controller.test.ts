...
describe('PlayListController', () => {
    ...
    it('throws not found when playlist does not exist', async () => {
        ...
        await expect(new PlayListController().getById('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('removes a song from a playlist', async () => {
        ...
        expect(response.message).toBe(messages.success.delete);
        expect(container.useCases.removeSongFromPlayList().execute).toHaveBeenCalledWith('pl-1', 'song-1');
    });
});
...
describe('SeasonsController', () => {
    ...
    it('updates a season', async () => {
        ...
        const response = await new SeasonsController().update('season-1', { name: 'Season 1 Updated' } as never);
