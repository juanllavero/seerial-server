...
jest.mock(
	"@/api/v1/shared/infrastructure/services/SanitizationService",
	() => ({
...
const sanitization = jest.requireMock(
	"@/api/v1/shared/infrastructure/services/SanitizationService",
);
...
describe("ImagesController", () => {
	beforeEach(() => {
		jest.clearAllMocks();
		container.imageProcessingService.getDirectoryListing.mockResolvedValue([
			{ name: "a.jpg" },
		]);
		container.imageProcessingService.getImageColorPalette.mockResolvedValue({
			primary: "#000000",
		});
		container.imageProcessingService.createTransparentImage.mockResolvedValue(
			Buffer.from("png"),
		);
	});
	...
	it("uploads an image to a sanitized destination", async () => {
		...
		expect(container.fileSystemService.writeImage).toHaveBeenCalledWith(
			"/tmp/uploads/cover.jpg",
			image.buffer,
		);
	...
	it("returns directory listing for sanitized path", async () => {
		const response = await new ImagesController().getDirectoryListing(
			"/media%2Fposters",
		);

		expect(
			container.imageProcessingService.getDirectoryListing,
		).toHaveBeenCalledWith("/media/posters");
	...
	it("streams local image with normalized dimensions", async () => {
		...
		await new ImagesController().getLocalImage(
			"resources/img/poster.jpg",
			320.9,
			180.2,
			request as never,
		);
		...
		expect(
			container.imageProcessingService.streamLocalImage,
	...
	it("streams remote image with optional dimensions", async () => {
		...
		await new ImagesController().getRemoteImage(
		...
		expect(
			container.imageProcessingService.streamRemoteImage,
		).toHaveBeenCalledWith({
	...
	it("gets color palette from local path and default options", async () => {
		const response = await new ImagesController().getImageColorPalette(
			undefined,
			"img/poster.jpg",
		);

		expect(
			container.imageProcessingService.getImageColorPalette,
		).toHaveBeenCalledWith("/external/resources/img/poster.jpg", {
	...
	it("rejects palette request without url or local path", async () => {
		await expect(
			new ImagesController().getImageColorPalette(),
	...
	it("creates transparent image and sets png content type", async () => {
		...
		const result = await controller.createTransparentImage(
			300,
			200,
			"https://example.com/img.jpg",
		);
		...
		expect(
			container.imageProcessingService.createTransparentImage,
		).toHaveBeenCalledWith("https://example.com/img.jpg", 300, 200);
	...
	it("rejects transparent image creation when required params are missing", async () => {
		await expect(
			new ImagesController().createTransparentImage(
				0,
				200,
				"https://example.com/img.jpg",
			),
