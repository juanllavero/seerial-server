...
describe('ImagesController', () => {
  ...
  it('returns directory listing for sanitized path', async () => {
    ...
    expect(container.imageProcessingService.getDirectoryListing).toHaveBeenCalledWith(
      '/media/posters',
    );
  ...
  it('rejects palette request without url or local path', async () => {
    await expect(new ImagesController().getImageColorPalette()).rejects.toMatchObject({
      statusCode: 400,
    });
  });
  ...
  it('rejects transparent image creation when required params are missing', async () => {
    await expect(
      new ImagesController().createTransparentImage(0, 200, 'https://example.com/img.jpg'),
    ).rejects.toMatchObject({ statusCode: 400 });
  });
});
