...
describe('DownloaderServiceImpl', () => {
    ...
    it('builds download video command through downloadContent', async () => {
        const service = new DownloaderServiceImpl();
        const spy = jest.spyOn(service as never, 'downloadContent').mockResolvedValue(undefined);

        await service.downloadVideo('https://example.com/v', 'downloads', 'video-file');
    ...
    it('logs and continues when deleting existing output files fails', async () => {
        ...
        const contentSpy = jest.spyOn(service as never, 'downloadContent').mockResolvedValue(undefined);
    ...
    it('auto-downloads first audio result when available', async () => {
        ...
        jest.spyOn(service, 'searchVideos').mockResolvedValue([
            { id: '1', title: 'x', url: 'https://example.com/a', duration: 10, thumbnail: '' },
        ]);
        const audioSpy = jest.spyOn(service, 'downloadAudio').mockResolvedValue(undefined);

        await service.autoDownloadFirstAudioResult('query', 'element-1');

        expect(audioSpy).toHaveBeenCalledWith('https://example.com/a', expect.any(String), 'element-1');
    });
    ...
    it('broadcasts completion and error events from spawned process', async () => {
        ...
        processEmitter.stderr.emit('data', Buffer.from('WARNING: test\nERROR: test-error\ninfo line'));
    ...
    it('returns parsed search results from yt-dlp json output', async () => {
        const service = new DownloaderServiceImpl();
        execMock.mockImplementation((_cmd: string, optionsOrCb: unknown, maybeCb?: (err: unknown, stdout?: string, stderr?: string) => void) => {
            const cb = (typeof optionsOrCb === 'function' ? optionsOrCb : maybeCb) as (err: unknown, stdout?: string, stderr?: string) => void;
            cb(
                null,
                {
    ...
    it('returns empty list when yt-dlp output is empty or execution fails', async () => {
        ...
        execMock.mockImplementationOnce((_cmd: string, optionsOrCb: unknown, maybeCb?: (err: unknown, stdout?: string, stderr?: string) => void) => {
            const cb = (typeof optionsOrCb === 'function' ? optionsOrCb : maybeCb) as (err: unknown, stdout?: string, stderr?: string) => void;
    ...
    it('handles synchronous spawn failures in downloadContent', async () => {
        ...
        await expect((service as never).downloadContent('cmd', 'file')).resolves.toBeUndefined();
    });
    ...
    it('covers downloadYoutubeDownloader early return and error branches in isolated module', async () => {
        ...
        jest.doMock('node:fs', () => {
            ...
            return {
                ...
                createWriteStream: jest.fn(() => {
                    const emitter = new EventEmitter() as EventEmitter & { close: (cb: () => void) => void };
        ...
        const isolated = await import('@/api/v1/shared/infrastructure/adapters/downloader/DownloaderServiceImpl');
        ...
        httpsGet.mockImplementation((_url: string, cb: (res: { statusCode: number; pipe: (f: unknown) => void }) => void) => {
            cb({ statusCode: 500, pipe: () => undefined });
            return { on: jest.fn().mockReturnThis() };
        });

        await expect(service.downloadYoutubeDownloader()).rejects.toThrow('Error downloading yt-dlp');
    });
});
