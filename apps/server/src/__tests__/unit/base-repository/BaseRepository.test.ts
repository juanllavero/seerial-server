...
describe('BaseRepository', () => {
  ...
  describe('validateString', () => {
    ...
    it('throws a 400 error when value is too short', () => {
      expect(() => repo.testValidateString('hi', 'name', { minLength: 5 })).toThrow(
        expect.objectContaining({ statusCode: BAD_REQUEST }),
      );
    });

    it('throws a 400 error when value is too long', () => {
      expect(() => repo.testValidateString('verylongstring', 'name', { maxLength: 5 })).toThrow(
        expect.objectContaining({ statusCode: BAD_REQUEST }),
      );
    });

    it('throws a 400 error when pattern does not match', () => {
      expect(() => repo.testValidateString('abc123', 'name', { pattern: /^\d+$/ })).toThrow(
        expect.objectContaining({ statusCode: BAD_REQUEST }),
      );
    });

    it('accepts value matching the pattern', () => {
      expect(repo.testValidateString('12345', 'code', { pattern: /^\d+$/ })).toBe('12345');
    });
  });
  ...
  describe('handleRepositoryError', () => {
    it('returns result of successful operation', async () => {
      const result = await repo.testHandleRepositoryError(async () => 'ok', 'error message');
