...
function buildSongsRepo(
	overrides: Partial<SongsRepositoryPort> = {},
): SongsRepositoryPort {
	return {
		findById: jest.fn(),
		findByPath: jest.fn(),
...
describe("Song use cases", () => {
	describe("CreateSongUseCase", () => {
		...
		it("returns null when the repository does not create a song", async () => {
			...
			await expect(
				new CreateSongUseCase(repo).execute({ title: "Missing" }),
			).resolves.toBeNull();
		});
	});
	...
	describe("FindSongByIdUseCase", () => {
		...
		it("returns null when the song does not exist", async () => {
			...
			await expect(
				new FindSongByIdUseCase(repo).execute("missing"),
			).resolves.toBeNull();
		});
	});
	...
	describe("FindSongByPathUseCase", () => {
		it("returns a song by file path", async () => {
			...
			const result = await new FindSongByPathUseCase(repo).execute(
				song.fileSrc,
			);
	...
	describe("FindSongsByAlbumIdUseCase", () => {
		it("returns all songs for an album", async () => {
			const songs = [
				buildSong(),
				buildSong({ id: "song-2", title: "Aerodynamic" }),
			];
			const repo = buildSongsRepo({
				findByAlbum: jest.fn().mockResolvedValue(songs),
			});

			const result = await new FindSongsByAlbumIdUseCase(repo).execute(
				"album-1",
			);
	...
	describe("DeleteSongUseCase", () => {
		it("deletes a song through the repository", async () => {
			...
			await expect(
				new DeleteSongUseCase(repo).execute("song-1"),
