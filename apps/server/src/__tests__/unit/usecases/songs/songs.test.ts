...
describe('Song use cases', () => {
  describe('CreateSongUseCase', () => {
    it('creates a song through the repository', async () => {
      ...
      const repo = buildSongsRepo({ create: jest.fn().mockResolvedValue(song) });

      const result = await new CreateSongUseCase(repo).execute({ title: 'One More Time' });
    ...
    it('returns null when the repository does not create a song', async () => {
      const repo = buildSongsRepo({ create: jest.fn().mockResolvedValue(null) });
  ...
  describe('FindSongByIdUseCase', () => {
    it('returns a song by id', async () => {
      const song = buildSong();
      const repo = buildSongsRepo({ findById: jest.fn().mockResolvedValue(song) });
    ...
    it('returns null when the song does not exist', async () => {
      const repo = buildSongsRepo({ findById: jest.fn().mockResolvedValue(null) });
  ...
  describe('FindSongByPathUseCase', () => {
    it('returns a song by file path', async () => {
      const song = buildSong();
      const repo = buildSongsRepo({ findByPath: jest.fn().mockResolvedValue(song) });
  ...
  describe('FindSongsByAlbumIdUseCase', () => {
    it('returns all songs for an album', async () => {
      ...
      const repo = buildSongsRepo({ findByAlbum: jest.fn().mockResolvedValue(songs) });
  ...
  describe('UpdateSongUseCase', () => {
    it('updates a song through the repository', async () => {
      ...
      const repo = buildSongsRepo({ update: jest.fn().mockResolvedValue(song) });

      const result = await new UpdateSongUseCase(repo).execute('song-1', { title: 'Updated Song' });

      expect(result).toEqual(song);
      expect(repo.update).toHaveBeenCalledWith('song-1', { title: 'Updated Song' });
    });
  });

  describe('DeleteSongUseCase', () => {
    it('deletes a song through the repository', async () => {
      const repo = buildSongsRepo({ delete: jest.fn().mockResolvedValue(undefined) });
