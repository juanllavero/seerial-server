...
describe('Artist use cases', () => {
  describe('AddArtistUseCase', () => {
    it('adds an artist through the repository', async () => {
      ...
      const repo = buildArtistsRepo({ add: jest.fn().mockResolvedValue(artist) });

      const result = await new AddArtistUseCase(repo).execute({ name: 'Daft Punk' });
  ...
  describe('GetArtistByIdUseCase', () => {
    it('returns an artist by id', async () => {
      const artist = buildArtist();
      const repo = buildArtistsRepo({ getById: jest.fn().mockResolvedValue(artist) });
    ...
    it('returns null when the artist does not exist', async () => {
      const repo = buildArtistsRepo({ getById: jest.fn().mockResolvedValue(null) });
  ...
  describe('UpdateArtistUseCase', () => {
    it('updates an artist through the repository', async () => {
      ...
      const repo = buildArtistsRepo({ update: jest.fn().mockResolvedValue(artist) });
  ...
  describe('DeleteArtistUseCase', () => {
    it('deletes the artist through the repository', async () => {
      const repo = buildArtistsRepo({ delete: jest.fn().mockResolvedValue(true) });
