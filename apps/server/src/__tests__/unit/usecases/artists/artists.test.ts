...
function buildArtistsRepo(
	overrides: Partial<ArtistsRepositoryPort> = {},
): ArtistsRepositoryPort {
	return {
		getById: jest.fn(),
		getByName: jest.fn(),
		add: jest.fn(),
...
describe("Artist use cases", () => {
	...
	describe("GetArtistByIdUseCase", () => {
		...
		it("returns null when the artist does not exist", async () => {
			...
			await expect(
				new GetArtistByIdUseCase(repo).execute("missing"),
			).resolves.toBeNull();
		});
	});
	...
	describe("DeleteArtistUseCase", () => {
		it("deletes the artist through the repository", async () => {
			...
			await expect(
				new DeleteArtistUseCase(repo).execute("artist-1"),
