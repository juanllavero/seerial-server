...
function buildServersRepo(
	overrides: Partial<ServersRepositoryPort> = {},
): ServersRepositoryPort {
	return {
...
describe("Server use cases", () => {
	describe("CreateServerUseCase", () => {
		...
		it("returns null when no server config is created", async () => {
			...
			await expect(
				new CreateServerUseCase(repo).execute(buildServer()),
			).resolves.toBeNull();
		});
	});
	...
	describe("GetServerUseCase", () => {
		it("returns the current server config", async () => {
			const server = buildServer();
			const repo = buildServersRepo({
				getServerConfig: jest.fn().mockResolvedValue(server),
			});
