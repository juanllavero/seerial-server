...
function buildPlayListRepo(overrides: Partial<PlayListRepositoryPort> = {}): PlayListRepositoryPort {
    return {
        findAll: jest.fn(),
        findById: jest.fn(),
        create: jest.fn(),
...
describe('Playlist use cases', () => {
    describe('CreatePlayListUseCase', () => {
        it('builds a new playlist DTO and creates it through the repository', async () => {
            ...
            const repo = buildPlayListRepo({ create: jest.fn().mockResolvedValue(playList) });
    ...
    describe('FindAllPlayListsUseCase', () => {
        it('returns all playlists from the repository', async () => {
            const playLists = [buildPlayList(), buildPlayList({ id: 'playlist-2', title: 'Sleep' })];
            const repo = buildPlayListRepo({ findAll: jest.fn().mockResolvedValue(playLists) });
    ...
    describe('FindPlayListByIdUseCase', () => {
        it('returns a playlist by id', async () => {
            const playList = buildPlayList();
            const repo = buildPlayListRepo({ findById: jest.fn().mockResolvedValue(playList) });

            const result = await new FindPlayListByIdUseCase(repo).execute(playList.id);
        ...
        it('returns null when the playlist does not exist', async () => {
            const repo = buildPlayListRepo({ findById: jest.fn().mockResolvedValue(null) });

            await expect(new FindPlayListByIdUseCase(repo).execute('missing')).resolves.toBeNull();
        });
    });
    ...
    describe('UpdatePlayListUseCase', () => {
        it('updates the playlist through the repository', async () => {
            ...
            const repo = buildPlayListRepo({ update: jest.fn().mockResolvedValue(updatedPlayList) });

            const result = await new UpdatePlayListUseCase(repo).execute('playlist-1', {
    ...
    describe('DeletePlayListUseCase', () => {
        it('deletes the playlist through the repository', async () => {
            const repo = buildPlayListRepo({ delete: jest.fn().mockResolvedValue(undefined) });

            await expect(new DeletePlayListUseCase(repo).execute('playlist-1')).resolves.toBeUndefined();
    ...
    describe('AddSongToPlayListUseCase', () => {
        it('adds a song to the playlist', async () => {
            const repo = buildPlayListRepo({ addSongToPlaylist: jest.fn().mockResolvedValue(undefined) });

            await new AddSongToPlayListUseCase(repo).execute('playlist-1', 'song-1');

            expect(repo.addSongToPlaylist).toHaveBeenCalledWith('playlist-1', 'song-1');
        });
    });

    describe('RemoveSongFromPlayListUseCase', () => {
        it('removes a song from the playlist', async () => {
            const repo = buildPlayListRepo({ removeSongFromPlaylist: jest.fn().mockResolvedValue(undefined) });

            await new RemoveSongFromPlayListUseCase(repo).execute('playlist-1', 'song-1');

            expect(repo.removeSongFromPlaylist).toHaveBeenCalledWith('playlist-1', 'song-1');
        });
    });
});