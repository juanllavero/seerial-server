...
describe('Watch-list use cases', () => {
    ...
    describe('watch-list relation management use cases', () => {
        ...
        it('reads current episode, season and video from repository', async () => {
            ...
            await expect(new GetCurrentEpisodeUseCase(repo).execute('season-1', 'user-1')).resolves.toEqual(episode);
            await expect(new GetCurrentSeasonUseCase(repo).execute('series-1', 'user-1')).resolves.toEqual(season);
            await expect(new GetCurrentVideoUseCase(repo).execute('movie-1', 'user-1')).resolves.toEqual(video);
        ...
        it('updates and deletes watch-list rows through the repository', async () => {
            ...
            await expect(new UpdateWatchListUseCase(repo).execute('watch-list-1', { watched: true, timeWatched: 123 })).resolves.toEqual(watchList);
            await expect(new DeleteWatchListUseCase(repo).execute('watch-list-1')).resolves.toBeUndefined();
    ...
    describe('UpdateWatchStateUseCase', () => {
        ...
        it('rejects when the episode cannot be resolved', async () => {
            mockContainer.useCases.getVideoById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'video-1', episodeId: 'episode-1' }) });
            mockContainer.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('rejects when the season cannot be resolved', async () => {
            mockContainer.useCases.getVideoById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'video-1', episodeId: 'episode-1' }) });
            mockContainer.useCases.getEpisodeById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'episode-1', seasonId: 'season-1' }) });
            mockContainer.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('marks a movie as watched when all videos are complete and clears continue watching', async () => {
            ...
            mockContainer.useCases.getVideoById.mockReturnValue({ execute: jest.fn().mockResolvedValue(video) });
            mockContainer.useCases.getMoviebyId.mockReturnValue({ execute: jest.fn().mockResolvedValue(movie) });
            mockContainer.useCases.removeVideoFromContinueWatching.mockReturnValue({ execute: removeContinueWatchingExecute });
        ...
        it('removes the movie watch-list relation when progress falls below completion', async () => {
            ...
            mockContainer.useCases.addVideoToContinueWatching.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
        ...
        it('rejects when the movie cannot be resolved', async () => {
            mockContainer.useCases.getVideoById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'video-1', movieId: 'movie-1' }) });
            mockContainer.useCases.getMoviebyId.mockReturnValue({ execute: jest.fn().mockResolvedValue(null) });
        ...
        it('returns after persisting progress when no watch-list row exists yet', async () => {
            mockContainer.useCases.getVideoById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'video-1', movieId: 'movie-1' }) });
            mockContainer.useCases.getMoviebyId.mockReturnValue({
                execute: jest.fn().mockResolvedValue({ id: 'movie-1', videos: [{ id: 'video-1', watchLists: [] }] }),
            });
            mockContainer.useCases.addVideoToContinueWatching.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
        ...
        it('handles legacy delete-relationship error by forcing continue watching', async () => {
            ...
            mockContainer.useCases.getSeasonById.mockReturnValue({ execute: jest.fn().mockResolvedValue({ id: 'season-1', seriesId: 'series-1' }) });
            mockContainer.useCases.addVideoToContinueWatching.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
            mockContainer.useCases.setEpisodeWatchState.mockReturnValue({
                execute: jest.fn().mockRejectedValue(new Error('Failed to delete WatchList relationship for episode')),
            });
        ...
        it('does not force continue watching when season cannot be resolved in legacy flow', async () => {
            ...
            mockContainer.useCases.getSeasonById.mockReturnValue({
                execute: jest.fn().mockResolvedValueOnce({ id: 'season-1', seriesId: 'series-1' }).mockResolvedValueOnce(null),
            });
            mockContainer.useCases.addVideoToContinueWatching.mockReturnValue({ execute: addContinueExec });
