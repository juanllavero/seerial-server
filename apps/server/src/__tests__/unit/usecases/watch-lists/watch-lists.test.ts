...
describe("Watch-list use cases", () => {
	...
	describe("RemoveContinueWatchingVideoUseCase", () => {
		it("removes a continue-watching entry through the repository", async () => {
			...
			await new RemoveContinueWatchingVideoUseCase(repo).execute(
				"video-1",
				"user-1",
			);

			expect(repo.removeContinueWatchingVideo).toHaveBeenCalledWith(
				"video-1",
				"user-1",
			);
		});
	...
	describe("ClearContinueWatchingUseCase", () => {
		it("clears continue-watching entries through the repository", async () => {
			...
			expect(repo.clearContinueWatching).toHaveBeenCalledWith(
				"user-1",
				"series-1",
				undefined,
			);
	...
	describe("GetContinueWatchingVideosUseCase", () => {
		it("returns continue-watching videos for a user", async () => {
			...
			const result = await new GetContinueWatchingVideosUseCase(repo).execute(
				"user-1",
			);
	...
	describe("watch-list relation management use cases", () => {
		it("adds and removes movie/series/season/video items through the repository", async () => {
			...
			await new RemoveMovieFromWatchListUseCase(repo).execute(
				"user-1",
				"movie-1",
			);
			await new RemoveSeriesFromWatchListUseCase(repo).execute(
				"user-1",
				"series-1",
			);
			await new RemoveSeasonFromWatchListUseCase(repo).execute(
				"user-1",
				"season-1",
			);
			await new RemoveVideoFromWatchListUseCase(repo).execute(
				"user-1",
				"video-1",
			);
		...
		it("reads current episode, season and video from repository", async () => {
			...
			await expect(
				new GetCurrentVideoUseCase(repo).execute("movie-1", "user-1"),
			).resolves.toEqual(video);

			expect(repo.findCurrentEpisode).toHaveBeenCalledWith(
				"season-1",
				"user-1",
			);
		...
		it("updates and deletes watch-list rows through the repository", async () => {
			...
			expect(repo.update).toHaveBeenCalledWith("watch-list-1", {
				watched: true,
				timeWatched: 123,
			});
	...
	describe("UpdateWatchStateUseCase", () => {
		...
		it("marks an episode as watched and persists video progress", async () => {
			...
			const setEpisodeWatchStateExecute = jest
				.fn()
				.mockResolvedValue(undefined);

			mockContainer.useCases.getVideoById.mockReturnValue({
				execute: getVideoByIdExecute,
			});
			mockContainer.useCases.getEpisodeById.mockReturnValue({
			...
			expect(setEpisodeWatchStateExecute).toHaveBeenCalledWith(
				"episode-1",
				"user-1",
				true,
			);
		...
		it("rejects when the episode cannot be resolved", async () => {
			mockContainer.useCases.getVideoById.mockReturnValue({
				execute: jest
					.fn()
					.mockResolvedValue({ id: "video-1", episodeId: "episode-1" }),
			});
			mockContainer.useCases.getEpisodeById.mockReturnValue({
				execute: jest.fn().mockResolvedValue(null),
			});
		...
		it("rejects when the season cannot be resolved", async () => {
			...
			mockContainer.useCases.getEpisodeById.mockReturnValue({
				execute: jest
					.fn()
					.mockResolvedValue({ id: "episode-1", seasonId: "season-1" }),
			});
			mockContainer.useCases.getSeasonById.mockReturnValue({
				execute: jest.fn().mockResolvedValue(null),
			});
		...
		it("marks a movie as watched when all videos are complete and clears continue watching", async () => {
			...
			const persistedWatchList = buildWatchList();
			const removeContinueWatchingExecute = jest
				.fn()
				.mockResolvedValue(undefined);
			...
			expect(repo.addMovie).toHaveBeenCalledWith("user-1", "movie-1");
			expect(removeContinueWatchingExecute).toHaveBeenCalledWith(
				"video-2",
				"user-1",
			);
		});
		...
		it("rejects when the movie cannot be resolved", async () => {
			mockContainer.useCases.getVideoById.mockReturnValue({
				execute: jest
					.fn()
					.mockResolvedValue({ id: "video-1", movieId: "movie-1" }),
			});
			mockContainer.useCases.getMoviebyId.mockReturnValue({
				execute: jest.fn().mockResolvedValue(null),
			});
		...
		it("handles legacy delete-relationship error by forcing continue watching", async () => {
			...
			mockContainer.useCases.setEpisodeWatchState.mockReturnValue({
				execute: jest
					.fn()
					.mockRejectedValue(
						new Error("Failed to delete WatchList relationship for episode"),
					),
			});
			...
			expect(
				mockContainer.useCases.addVideoToContinueWatching,
			).toHaveBeenCalled();
		});
		...
		it("does not force continue watching when episode cannot be resolved in legacy flow", async () => {
			...
			const addContinueExec = jest
				...
				.mockRejectedValueOnce(
					new Error("Failed to delete WatchList relationship"),
				)
		...
		it("forces continue watching for movie videos in legacy flow", async () => {
			...
			expect(
				mockContainer.useCases.addVideoToContinueWatching().execute,
			).toHaveBeenCalledWith("video-1", "user-1", undefined, "movie-1");
		});
	});
});
