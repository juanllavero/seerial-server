...
describe('Series use cases', () => {
  ...
  it('creates a series through the repository', async () => {
    ...
    await expect(new CreateSeriesUseCase(repo).execute({ name: 'Lost' } as never)).resolves.toEqual(
      series,
    );
  });
  ...
  it('throws when deleting a missing series', async () => {
    ...
    await expect(new DeleteSeriesUseCase(repo).execute('missing')).rejects.toMatchObject({
      statusCode: 404,
    });
  });
