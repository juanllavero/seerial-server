...
describe("Series use cases", () => {
	beforeEach(() => {
		...
		mockContainer.useCases.deleteSeriesData.mockReturnValue({
			execute: jest.fn().mockResolvedValue(undefined),
		});
		mockContainer.useCases.getLibrary.mockReturnValue({
			execute: jest.fn().mockResolvedValue({ id: "library-1" }),
		});
		mockContainer.useCases.removeAnalyzedFolder.mockReturnValue({
			execute: jest.fn().mockResolvedValue(undefined),
		});
	});
	...
	it("creates a series through the repository", async () => {
		...
		const repo = buildSeriesRepo({
			create: jest.fn().mockResolvedValue(series),
		});

		await expect(
			new CreateSeriesUseCase(repo).execute({ name: "Lost" } as never),
	...
	it("finds a series by id with include option", async () => {
		...
		const result = await new FindSeriesByIdUseCase(repo).execute(
			"series-1",
			"few",
		);
	...
	it("updates a series through the repository", async () => {
		...
		const repo = buildSeriesRepo({
			update: jest.fn().mockResolvedValue(series),
		});

		await expect(
			new UpdateSeriesUseCase(repo).execute("series-1", {
				name: "Updated Lost",
			} as never),
		).resolves.toEqual(series);
	});
	...
	it("throws when deleting a missing series", async () => {
		...
		await expect(
			new DeleteSeriesUseCase(repo).execute("missing"),
	...
	it("deletes seasons, series data, analyzed folder, and the series row when library exists", async () => {
		...
		mockContainer.useCases.deleteSeason.mockReturnValue({
			execute: deleteSeasonExecute,
		});
		mockContainer.useCases.deleteSeriesData.mockReturnValue({
			execute: deleteSeriesDataExecute,
		});
		mockContainer.useCases.getLibrary.mockReturnValue({
			execute: getLibraryExecute,
		});
		mockContainer.useCases.removeAnalyzedFolder.mockReturnValue({
			execute: removeAnalyzedFolderExecute,
		});
		...
		expect(removeAnalyzedFolderExecute).toHaveBeenCalledWith(
			"library-1",
			"/shows/series-1",
		);
