...
describe('Movie use cases', () => {
    beforeEach(() => {
        ...
        mockContainer.useCases.deleteMovieData.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    });

    describe('FindMovieByIdUseCase', () => {
        it('returns the movie from the repository', async () => {
            const movie = buildMovie();
            const repo = buildMoviesRepo({ findById: jest.fn().mockResolvedValue(movie) });
        ...
        it('returns null when the movie does not exist', async () => {
            const repo = buildMoviesRepo({ findById: jest.fn().mockResolvedValue(null) });
    ...
    describe('FindMovieByPathUseCase', () => {
        it('delegates movie lookup by path', async () => {
            const movie = buildMovie();
            const repo = buildMoviesRepo({ findByPath: jest.fn().mockResolvedValue(movie) });

            const result = await new FindMovieByPathUseCase(repo).execute('/media/movies/movie-1/file.mkv');
    ...
    describe('UpdateMovieUseCase', () => {
        it('updates the movie through the repository', async () => {
            ...
            const repo = buildMoviesRepo({ update: jest.fn().mockResolvedValue(movie) });

            const result = await new UpdateMovieUseCase(repo).execute('movie-1', { name: 'Updated Movie' } as never);

            expect(result).toEqual(movie);
            expect(repo.update).toHaveBeenCalledWith('movie-1', { name: 'Updated Movie' });
        });
    });

    describe('DeleteMovieUseCase', () => {
        it('throws when the movie does not exist', async () => {
            const moviesRepo = buildMoviesRepo({ findById: jest.fn().mockResolvedValue(null) });
        ...
        it('deletes child videos, movie data, analyzed folder, and the movie row', async () => {
            ...
            mockContainer.useCases.deleteMovieData.mockReturnValue({ execute: deleteMovieDataExecute });
            ...
            expect(librariesRepo.removeAnalyzedFolder).toHaveBeenCalledWith(movie.libraryId, movie.folder);
