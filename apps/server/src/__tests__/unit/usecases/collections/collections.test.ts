...
describe("Collection use cases", () => {
	describe("CreateCollectionUseCase", () => {
		it("creates a collection through the repository", async () => {
			...
			const repo = buildCollectionsRepo({
				add: jest.fn().mockResolvedValue(collection),
			});

			const result = await new CreateCollectionUseCase(repo).execute({
				title: "Favorites",
			});
	...
	describe("FindCollectionByIdUseCase", () => {
		it("returns the collection by id", async () => {
			...
			const result = await new FindCollectionByIdUseCase(repo).execute(
				collection.id,
			);
		...
		it("returns null when the collection does not exist", async () => {
			...
			await expect(
				new FindCollectionByIdUseCase(repo).execute("missing"),
			).resolves.toBeNull();
		});
	});
	...
	describe("UpdateCollectionUseCase", () => {
		it("updates the collection through the repository", async () => {
			...
			const repo = buildCollectionsRepo({
				update: jest.fn().mockResolvedValue(collection),
			});

			const result = await new UpdateCollectionUseCase(repo).execute(
				"collection-1",
				{
	...
	describe("DeleteCollectionUseCase", () => {
		it("returns the repository deletion result", async () => {
			...
			await expect(
				new DeleteCollectionUseCase(repo).execute("collection-1"),
			).resolves.toBe(true);
	...
	describe("ReorderCollectionItemsUseCase", () => {
		it("delegates item ordering to the repository", async () => {
			...
			const repo = buildCollectionsRepo({
				reorderContent: jest.fn().mockResolvedValue(undefined),
			});

			await new ReorderCollectionItemsUseCase(repo).execute(
				"collection-1",
				orderedItems,
			);

			expect(repo.reorderContent).toHaveBeenCalledWith(
				"collection-1",
				orderedItems,
			);
		});
	...
	describe("Additional collection use cases", () => {
		it("adds an album to a collection through the repository", async () => {
			const repo = buildCollectionsRepo({
				addAlbum: jest.fn().mockResolvedValue(undefined),
			});

			await expect(
				new AddAlbumToCollectionUseCase(repo).execute(
					"collection-1",
					"album-1",
				),
		...
		it("adds a collection to a library through the repository", async () => {
			const repo = buildCollectionsRepo({
				addLibrary: jest.fn().mockResolvedValue(undefined),
			});

			await expect(
				new AddLibraryToCollectionUseCase(repo).execute(
					"library-1",
					"collection-1",
				),
			).resolves.toBeUndefined();
		...
		it("finds collections in a specific library", async () => {
			...
			const repo = buildCollectionsRepo({
				getAll: jest.fn().mockResolvedValue(collections),
			});

			await expect(
				new FindCollectionsInLibraryUseCase(repo).execute("library-1"),
		...
		it("returns music extras for a collection", async () => {
			...
			await expect(
				new GetMusicExtrasUseCase().execute("collection-1"),
