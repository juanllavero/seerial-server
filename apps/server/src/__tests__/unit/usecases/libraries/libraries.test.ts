...
function buildMoviesRepo(
	overrides: Partial<MoviesRepositoryPort> = {},
...
function buildAlbumsRepo(
	overrides: Partial<AlbumsRepositoryPort> = {},
): AlbumsRepositoryPort {
	return {
		findAll: jest.fn().mockResolvedValue([]),
		findById: jest.fn(),
		create: jest.fn(),
...
describe("Libraries use cases", () => {
	describe("GetLibrariesUseCase", () => {
		it("returns all libraries", async () => {
			const libs = [
				buildLibrary(),
				buildLibrary({ id: "lib-2", name: "TV Shows" }),
			];
			const repo = buildLibrariesRepo({
				getAll: jest.fn().mockResolvedValue(libs),
			});
	...
	describe("GetLibraryUseCase", () => {
		it("returns library by id", async () => {
			...
			const repo = buildLibrariesRepo({
				getById: jest.fn().mockResolvedValue(lib),
			});
		...
		it("returns library by video id", async () => {
			const lib = buildLibrary();
			const repo = buildLibrariesRepo({
				getByVideoId: jest.fn().mockResolvedValue(lib),
			});

			const result = await new GetLibraryByVideoIdUseCase(repo).execute(
				"video-1",
			);
	...
	describe("UpdateLibraryUseCase", () => {
		it("delegates to the repository and returns the updated library", async () => {
			...
			const result = await new UpdateLibraryUseCase(repo).execute("lib-1", {
				name: "Renamed",
			});
	...
	describe("ReorderLibrariesUseCase", () => {
		it("calls reorder with ordered ids abd returns the result", async () => {
			const repo = buildLibrariesRepo({
				reorder: jest.fn().mockResolvedValue(true),
			});

			const result = await new ReorderLibrariesUseCase(repo).execute([
				"lib-2",
				"lib-1",
			]);
	...
	describe("ReorderLibraryItemsUseCase", () => {
		it("delegates item reordering to repository", async () => {
			...
			const result = await new ReorderLibraryItemsUseCase(repo).execute(
				"lib-1",
				orderedItems,
			);
	...
	describe("Analyzed files and folders use cases", () => {
		it("adds and removes analyzed files through the repository", async () => {
			...
			await expect(
				new AddAnalyzedFileUseCase(repo).execute(
					"lib-1",
					"/movies/a.mp4",
					"video-1",
				),
			...
			await expect(
				new RemoveAnalyzedFileUseCase(repo).execute("lib-1", "/movies/a.mp4"),
			).resolves.toEqual(library);

			expect(repo.addAnalyzedFile).toHaveBeenCalledWith(
				"lib-1",
				"/movies/a.mp4",
				"video-1",
			);
			expect(repo.removeAnalyzedFile).toHaveBeenCalledWith(
				"lib-1",
				"/movies/a.mp4",
			);
		});
		...
		it("adds and removes analyzed folders through the repository", async () => {
			...
			await expect(
				new AddAnalyzedFolderUseCase(repo).execute(
					"lib-1",
					"/series/show",
					"video-1",
				),
			...
			await expect(
				new RemoveAnalyzedFolderUseCase(repo).execute("lib-1", "/series/show"),
			).resolves.toEqual(library);

			expect(repo.addAnalyzedFolder).toHaveBeenCalledWith(
				"lib-1",
				"/series/show",
				"video-1",
			);
			expect(repo.removeAnalyzedFolder).toHaveBeenCalledWith(
				"lib-1",
				"/series/show",
			);
		});
