...
describe("Album use cases", () => {
	describe("CreateAlbumUseCase", () => {
		it("creates an album through the repository", async () => {
			...
			const repo = buildAlbumsRepo({
				create: jest.fn().mockResolvedValue(album),
			});

			const result = await new CreateAlbumUseCase(repo).execute({
				title: "Discovery",
			});
	...
	describe("FindAlbumByIdUseCase", () => {
		it("returns an album by id", async () => {
			const album = buildAlbum();
			const repo = buildAlbumsRepo({
				findById: jest.fn().mockResolvedValue(album),
			});
		...
		it("returns null when the album does not exist", async () => {
			...
			await expect(
				new FindAlbumByIdUseCase(repo).execute("missing"),
			).resolves.toBeNull();
		});
	});
	...
	describe("FindAllAlbumsUseCase", () => {
		it("returns all albums for a library", async () => {
			const albums = [
				buildAlbum(),
				buildAlbum({ id: "album-2", title: "Random Access Memories" }),
			];
			const repo = buildAlbumsRepo({
				findAll: jest.fn().mockResolvedValue(albums),
			});
	...
	describe("UpdateAlbumUseCase", () => {
		it("updates an album through the repository", async () => {
			...
			const repo = buildAlbumsRepo({
				update: jest.fn().mockResolvedValue(album),
			});
	...
	describe("DeleteAlbumUseCase", () => {
		it("deletes the album through the repository", async () => {
			...
			await expect(
				new DeleteAlbumUseCase(repo).execute("album-1"),
	...
	describe("AddArtistToAlbumUseCase", () => {
		it("links an artist to an album through the repository", async () => {
			...
			const repo = buildAlbumsRepo({
				addArtistToAlbum: jest.fn().mockResolvedValue(relation),
			});

			const result = await new AddArtistToAlbumUseCase(repo).execute(
				"artist-1",
				"album-1",
			);
