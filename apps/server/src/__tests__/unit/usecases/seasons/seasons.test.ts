...
describe('Season use cases', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockContainer.useCases.deleteEpisode.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
        mockContainer.useCases.deleteSeasonData.mockReturnValue({ execute: jest.fn().mockResolvedValue(undefined) });
    });

    it('creates a season through the repository', async () => {
        const season = buildSeason();
        const repo = buildSeasonRepo({ create: jest.fn().mockResolvedValue(season) });

        await expect(new CreateSeasonUseCase(repo).execute({ seasonNumber: 1 } as never)).resolves.toEqual(season);
    });

    it('finds a season by id with include option', async () => {
        const season = buildSeason();
        const repo = buildSeasonRepo({ findById: jest.fn().mockResolvedValue(season) });
    ...
    it('returns all seasons for a library/series scope', async () => {
        ...
        const repo = buildSeasonRepo({ findAll: jest.fn().mockResolvedValue(seasons) });
    ...
    it('returns seasons by series id', async () => {
        ...
        const repo = buildSeasonRepo({ findSeasonsBySeriesId: jest.fn().mockResolvedValue(seasons) });

        await expect(new FindSeasonsBySeriesIdUseCase(repo).execute('series-1')).resolves.toEqual(seasons);
    });

    it('updates a season through the repository', async () => {
        const season = buildSeason({ seasonNumber: 2 });
        const repo = buildSeasonRepo({ update: jest.fn().mockResolvedValue(season) });

        await expect(new UpdateSeasonUseCase(repo).execute('season-1', { seasonNumber: 2 } as never)).resolves.toEqual(season);
    });

    it('throws when deleting a missing season', async () => {
        const repo = buildSeasonRepo({ findById: jest.fn().mockResolvedValue(null) });

        await expect(new DeleteSeasonUseCase(repo).execute('missing')).rejects.toMatchObject({ statusCode: 404 });
    });
    ...
    it('deletes child episodes, season data, and the season row', async () => {
        ...
        mockContainer.useCases.deleteEpisode.mockReturnValue({ execute: deleteEpisodeExecute });
        mockContainer.useCases.deleteSeasonData.mockReturnValue({ execute: deleteSeasonDataExecute });
