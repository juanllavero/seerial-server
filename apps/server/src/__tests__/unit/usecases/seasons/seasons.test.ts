...
describe("Season use cases", () => {
	...
	it("finds a season by id with include option", async () => {
		...
		const result = await new FindSeasonByIdUseCase(repo).execute(
			"season-1",
			"all",
		);
	...
	it("returns all seasons for a library/series scope", async () => {
		const seasons = [
			buildSeason(),
			buildSeason({ id: "season-2", seasonNumber: 2 }),
		];
		const repo = buildSeasonRepo({
		...
		await expect(
			new FindAllSeasonsUseCase(repo).execute("series-1"),
		).resolves.toEqual(seasons);
	...
	it("returns seasons by series id", async () => {
		...
		await expect(
			new FindSeasonsBySeriesIdUseCase(repo).execute("series-1"),
		).resolves.toEqual(seasons);
	});
	...
	it("throws when deleting a missing season", async () => {
		...
		await expect(
			new DeleteSeasonUseCase(repo).execute("missing"),
