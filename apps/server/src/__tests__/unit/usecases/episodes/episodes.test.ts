...
function buildSeasonRepo(
	overrides: Partial<SeasonsRepositoryPort> = {},
): SeasonsRepositoryPort {
	return {
		findAll: jest.fn(),
		findById: jest.fn(),
...
function buildSeriesRepo(
	overrides: Partial<SeriesRepositoryPort> = {},
...
function buildVideoRepo(
	overrides: Partial<VideoRepositoryPort> = {},
): VideoRepositoryPort {
	return {
		findById: jest.fn(),
...
describe("Episode use cases", () => {
	...
	describe("CreateEpisodeUseCase", () => {
		it("creates an episode through the repository", async () => {
			...
			const result = await new CreateEpisodeUseCase(repo).execute({
				title: "Pilot",
			} as never);
	...
	describe("FindEpisodeByPathUseCase", () => {
		it("returns an episode by video source path", async () => {
			const episode = buildEpisode();
			const repo = buildEpisodeRepo({
				findByVideoSrc: jest.fn().mockResolvedValue(episode),
			});

			const result = await new FindEpisodeByPathUseCase(repo).execute(
				"/shows/show/pilot.mkv",
			);
	...
	describe("FindEpisodesBySeasonIdUseCase", () => {
		it("returns episodes for a season", async () => {
			const episodes = [
				buildEpisode(),
				buildEpisode({ id: "episode-2", episodeNumber: 2 }),
			];
			const repo = buildEpisodeRepo({
				findAllBySeasonId: jest.fn().mockResolvedValue(episodes),
			});

			const result = await new FindEpisodesBySeasonIdUseCase(repo).execute(
				"season-1",
			);
	...
	describe("UpdateEpisodeUseCase", () => {
		it("updates an episode through the repository", async () => {
			...
			const result = await new UpdateEpisodeUseCase(repo).execute("episode-1", {
				title: "Updated Pilot",
			} as never);

			expect(result).toEqual(episode);
			expect(repo.update).toHaveBeenCalledWith("episode-1", {
				title: "Updated Pilot",
			});
		});
	});
	...
	describe("DeleteEpisodeUseCase", () => {
		it("throws when the episode does not exist", async () => {
			...
			await expect(
				new DeleteEpisodeUseCase(repo).execute("missing"),
	...
	describe("SetEpisodeWatchStateUseCase", () => {
		...
		it("marks a one-episode season as watched and advances series state", async () => {
			...
			const seriesRepo = buildSeriesRepo({
				findById: jest
					.fn()
					.mockResolvedValue({ id: "series-1", seasons: [{ id: "season-1" }] }),
			});
			...
			expect(watchListRepo.addSeason).toHaveBeenCalledWith(
				"user-1",
				"season-1",
			);
			expect(watchListRepo.clearContinueWatching).toHaveBeenCalledWith(
				"user-1",
				"series-1",
			);
			expect(watchListRepo.addSeries).toHaveBeenCalledWith(
				"user-1",
				"series-1",
			);
		});
		...
		it("clears season and keeps continue watching when marking as not watched", async () => {
			...
			const videoRepo = buildVideoRepo({
				findByEpisodeId: jest
					.fn()
					.mockImplementation(async (id: string) => ({ id: `video-${id}` })),
			});
			const watchListRepo = buildWatchListRepo({
				addVideo: jest.fn().mockResolvedValue(undefined),
			...
			expect(watchListRepo.removeSeason).toHaveBeenCalledWith(
				"user-1",
				"season-1",
			);
			expect(watchListRepo.removeSeries).toHaveBeenCalledWith(
				"user-1",
				"series-1",
			);
		...
		it("updates previous and future seasons and skips continue-watching insertion when the next video is missing", async () => {
			const targetEpisode = buildEpisode({
				id: "episode-2-1",
				seasonId: "season-2",
				episodeNumber: 1,
			});
			const episodesById = {
				"episode-1-1": { id: "episode-1-1", episodeNumber: 1 },
				"episode-2-1": { id: "episode-2-1", episodeNumber: 1 },
				"episode-2-2": { id: "episode-2-2", episodeNumber: 2 },
				"episode-3-1": { id: "episode-3-1", episodeNumber: 1 },
			};
			const seasonsById = {
				"season-1": {
					id: "season-1",
					seriesId: "series-1",
					seasonNumber: 1,
					episodes: [{ id: "episode-1-1" }],
				},
				"season-2": {
					id: "season-2",
					seriesId: "series-1",
					seasonNumber: 2,
					episodes: [{ id: "episode-2-1" }, { id: "episode-2-2" }],
				},
				"season-3": {
					id: "season-3",
					seriesId: "series-1",
					seasonNumber: 3,
					episodes: [{ id: "episode-3-1" }],
				},
			};

			const episodeRepo = buildEpisodeRepo({
				findById: jest.fn().mockImplementation(async (id: string) => {
					if (id === targetEpisode.id) {
						return targetEpisode;
					}

					return (
						(
							episodesById as Record<
								string,
			...
			const videoRepo = buildVideoRepo({
				findByEpisodeId: jest.fn().mockImplementation(async (id: string) => {
					if (id === "episode-2-2") {
						return null;
					}
			...
			await new SetEpisodeWatchStateUseCase(
				...
				videoRepo,
				watchListRepo,
			).execute(targetEpisode.id, "user-1", true);

			expect(watchListRepo.addVideo).toHaveBeenCalledWith(
				"user-1",
				"video-episode-1-1",
			);
			expect(watchListRepo.addVideo).toHaveBeenCalledWith(
			...
			expect(watchListRepo.removeVideo).toHaveBeenCalledWith(
				"user-1",
				"video-episode-3-1",
			);
			expect(watchListRepo.addSeason).toHaveBeenCalledWith(
			...
			expect(watchListRepo.addContinueWatchingVideo).not.toHaveBeenCalled();
		});
	});
});
