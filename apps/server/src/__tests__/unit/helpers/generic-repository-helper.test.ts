...
describe("GenericRepositoryHelper", () => {
	...
	it("findManyByField returns all entities from model.find", async () => {
		...
		await expect(helper.findManyByField("name", "A")).resolves.toEqual(
			entities,
		);
	});
	...
	it("create returns existing entity when checkExisting hits by id", async () => {
		...
		await expect(
			helper.create({ id: "same-id", name: "New" }),
	...
	it("create maps duplicate database errors to a friendly message", async () => {
		...
		await expect(helper.create({ name: "Created" })).rejects.toThrow(
			"TestEntity already exists.",
		);
	});
	...
	it("createWithRelation delegates to create with the relation field", async () => {
		...
		expect(createSpy).toHaveBeenCalledWith(
			{ name: "Created", libraryId: "library-1" },
			false,
		);
	});
	...
	it("update preloads and saves entity", async () => {
		...
		const getRepository = jest
			.spyOn(TestEntity, "getRepository")
			.mockReturnValue({
				metadata: {
					columns: [],
					relations: [],
	...
	it("update strips relations and reapplies simple-json fields before saving", async () => {
		...
		const preloaded = {
			...
			save: jest
				.fn()
				.mockResolvedValue({ id: "id-1", metadata: { nested: true } }),
		};
	...
	it("delete preserves not-found errors", async () => {
		...
		await expect(helper.delete("id-1")).rejects.toThrow(
			"TestEntity with ID id-1 not found",
		);
	});
	...
	it("delete wraps unexpected errors", async () => {
		...
		await expect(helper.delete("id-1")).rejects.toThrow(
			"Failed to delete TestEntity with ID id-1",
		);
	});
	...
	it("findAll wraps repository failures", async () => {
		...
		await expect(helper.findAll()).rejects.toThrow(
			"Failed to retrieve all TestEntity",
		);
	});

	it("createRelationship returns the existing relation when found", async () => {
		const helper = createHelper();
		const relationModel = TestEntity as typeof BaseEntity &
			(new () => TestEntity);
	...
	it("createRelationship skips existing lookup when disabled", async () => {
		...
		await expect(
			helper.createRelationship(
				relationModel,
				{ name: "Created" } as Partial<TestEntity>,
				false,
			),
