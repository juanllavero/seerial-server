...
describe('GenericRepositoryHelper', () => {
    ...
    it('findById wraps repository failures', async () => {
        ...
        await expect(helper.findById('test-1')).rejects.toThrow('Failed to retrieve TestEntity with ID test-1');
    });
    ...
    it('findByField wraps repository failures', async () => {
        ...
        await expect(helper.findByField('name', 'missing')).rejects.toThrow('Failed to retrieve TestEntity by name');
    });
    ...
    it('findManyByField wraps repository failures', async () => {
        ...
        await expect(helper.findManyByField('name', 'A')).rejects.toThrow('Failed to retrieve TestEntity by name');
    });
    ...
    it('create generates a full UUID when short IDs are disabled', async () => {
        ...
        expect(createSpy).toHaveBeenCalledWith(expect.objectContaining({ id: expect.stringMatching(/-/) }));
    });
    ...
    it('createWithRelation delegates to create with the relation field', async () => {
        const helper = createHelper();
        const createSpy = jest.spyOn(helper, 'create').mockResolvedValue({ id: 'test-1', name: 'Created' } as TestEntity);

        await expect(helper.createWithRelation({ name: 'Created' }, 'libraryId', 'library-1', false)).resolves.toEqual({
            id: 'test-1',
            name: 'Created',
        });
    ...
    it('update wraps missing preload results', async () => {
        ...
        await expect(helper.update('id-1', { name: 'Updated' })).rejects.toThrow('Failed to update TestEntity with ID id-1');
    });
    ...
    it('createRelationship returns the existing relation when found', async () => {
        ...
        await expect(helper.createRelationship(relationModel, { name: 'Existing' } as Partial<TestEntity>)).resolves.toBe(existing);
    });

    it('createRelationship creates and saves a new relation when none exists', async () => {
        const helper = createHelper();
        const relationModel = TestEntity as typeof BaseEntity & (new () => TestEntity);
        const created = { id: 'rel-2', name: 'Created', save: jest.fn().mockResolvedValue(undefined) } as unknown as TestEntity;

        jest.spyOn(relationModel, 'findOne').mockResolvedValue(null);
        jest.spyOn(relationModel, 'create').mockReturnValue(created);

        const result = await helper.createRelationship(relationModel, { name: 'Created' } as Partial<TestEntity>);
    ...
    it('createRelationship skips existing lookup when disabled', async () => {
        const helper = createHelper();
        const relationModel = TestEntity as typeof BaseEntity & (new () => TestEntity);
        const created = { id: 'rel-3', name: 'Created', save: jest.fn().mockResolvedValue(undefined) } as unknown as TestEntity;
        const findOneSpy = jest.spyOn(relationModel, 'findOne');

        jest.spyOn(relationModel, 'create').mockReturnValue(created);

        await expect(helper.createRelationship(relationModel, { name: 'Created' } as Partial<TestEntity>, false)).resolves.toBe(created);
    ...
    it('createRelationship wraps persistence errors', async () => {
        ...
        await expect(helper.createRelationship(relationModel, { name: 'Broken' } as Partial<TestEntity>)).rejects.toThrow(
            'Failed to create TestEntity relationship',
        );
    });
    ...
    it('deleteRelationship deletes relation rows and wraps errors', async () => {
        const helper = createHelper();
        const relationModel = TestEntity as typeof BaseEntity & (new () => TestEntity);
        const deleteSpy = jest.spyOn(relationModel, 'delete').mockResolvedValue({ affected: 1 } as never);

        await expect(helper.deleteRelationship(relationModel, { id: 'rel-1' } as Partial<TestEntity>)).resolves.toBeUndefined();
        expect(deleteSpy).toHaveBeenCalledWith({ id: 'rel-1' });

        deleteSpy.mockRejectedValueOnce(new Error('boom'));

        await expect(helper.deleteRelationship(relationModel, { id: 'rel-1' } as Partial<TestEntity>)).rejects.toThrow(
            'Failed to delete TestEntity relationship',
        );
    });
});
