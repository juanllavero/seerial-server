...
describe("MediaInfoServiceImpl", () => {
	...
	it("decodes paths before delegating audio metadata extraction", async () => {
		...
		await expect(
			service.getAudioMetadata("C%3A%2Fmedia%2Ftrack.mp3"),
		).resolves.toEqual({
			codec: "aac",
		});
	...
	it("decodes paths before delegating chapter extraction", async () => {
		...
		await expect(
			service.getMediaChapters("C%3A%2Fmedia%2Fmovie.mkv"),
		).resolves.toEqual([{ title: "Intro" }]);
	...
	it("decodes paths before delegating media probing", async () => {
		...
		await expect(
			service.getMediaInformation("C%3A%2Fmedia%2Fmovie.mkv"),
		).resolves.toEqual({
