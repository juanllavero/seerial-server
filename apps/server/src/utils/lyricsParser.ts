import type { EnhancedLyricsLine, LyricWord, LyricsLine, PlainLyricsLine } from '@seerial/domain';
...
function parseElrcWords(rest: string): LyricWord[] {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (!text) continue;
        words.push({ text, startTime: time, endTime: tokens[i + 1]?.time ?? time + 0.5 });
    }
    return words;
}
...
function parseSpans(content: string): LyricWord[] {
    ...
    for (const m of content.matchAll(/<span\b([^>]*)>([\s\S]*?)<\/span>/g)) {
        ...
        words.push({ text, startTime: parseTtmlTime(begin), endTime: parseTtmlTime(end) });
    }
    return words;
}
...
export function buildLyricsFromTtml(
    ...
    for (let i = 0; i < bodyLines.length; i++) {
        ...
        result.push({
            ...
            words,
            ...(transTextLines[transIdx] ? { translation: transTextLines[transIdx] } : {}),
        });
        transIdx++;
