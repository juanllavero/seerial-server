import type { EnhancedLyricsLine, LyricsLine, LyricWord, PlainLyricsLine } from '@seerial/domain';
...
function parseTtmlBody(content: string): TtmlBodyLine[] {
  ...
  for (const m of bodyM[1].matchAll(/<p\b([^>]*)>([\s\S]*?)<\/p>/g)) {
    ...
    const plainText =
      words.length === 0 ? m[2].replace(/<[^>]+>/g, '').trim() || undefined : undefined;
    lines.push({
      startTime: parseTtmlTime(begin),
      endTime: parseTtmlTime(end),
      lineId,
...
function parseTtmlPronunciation(content: string): Map<string, LyricWord[]> {
  ...
  const transM = /<transliterations>([\s\S]*?)<\/transliterations>/.exec(content);
  if (!transM) return map;

  const firstM = /<transliteration\b[^>]*>([\s\S]*?)<\/transliteration>/.exec(transM[1]);
 ...
 */
function mapByOrigTiming(origWords: LyricWord[], pronWords: LyricWord[]): LyricWord[] {
...
function buildEnhancedLine(
  ...
  const pronWords = pron?.words?.length ? mapByOrigTiming(orig.words, pron.words) : undefined;
...
function buildPlainLine(
  ...
  const pronText =
    pron?.text ?? (pron?.words?.length ? pron.words.map((w) => w.text).join('') : undefined);
...
function parseTtmlTranslation(translationContent: string): string[] {
  const lines: string[] = [];
  for (const line of parseLrc(translationContent)) {
    if (!line.isBlank && line.text) lines.push(line.text);
  }
  return lines;
}

function buildTtmlLine(
  bl: TtmlBodyLine,
  pronMap: Map<string, LyricWord[]>,
  ...
  const pronunciation = pronWords?.length ? mapByOrigTiming(bl.words, pronWords) : undefined;
  const words: EnhancedLyricsLine = {
    original: bl.words,
...
export function buildLyricsFromTtml(
  ...
  const transTextLines = translationContent ? parseTtmlTranslation(translationContent) : [];
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    result.push(buildTtmlLine(bl, pronMap, transTextLines[transIdx]));
    transIdx++;

    const nextStart = bodyLines[i + 1]?.startTime;
    if (nextStart !== undefined && nextStart - bl.endTime > BLANK_GAP_THRESHOLD_S) {
...
export function buildLyricsFromLrc(
  ...
  for (const orig of origLines) {
    ...
    if (orig.words?.length) {
      result.push(buildEnhancedLine(orig as RawLrcLine & { words: LyricWord[] }, pron, trans));
    } else {
      result.push(buildPlainLine(orig as RawLrcLine & { text: string }, pron, trans));
    }
  }
