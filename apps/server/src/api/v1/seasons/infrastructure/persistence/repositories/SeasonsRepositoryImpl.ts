...
import type { SeasonsRepositoryPort } from '../../../application/ports/SeasonsRepositoryPort';
import type { Season } from '../../../domain/Season';
import { SeasonModel } from '../models/SeasonModel';

export class SeasonsRepositoryImpl extends BaseRepository implements SeasonsRepositoryPort {
  ...
  async findById(id: string, include = 'none'): Promise<Season | null> {
    ...
    const includeAll = ['episodes', 'episodes.video', 'episodes.video.watchLists', 'watchLists'];

    return this.helper.findById(validatedId, {
      relations: include === 'few' ? includeFew : include === 'all' ? includeAll : [],
    });
  }
