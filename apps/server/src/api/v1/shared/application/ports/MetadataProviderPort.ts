...
export interface MetadataProviderPort {
  ...
  // Get credits
  getMovieCredits(movieID: number, language: string): Promise<CreditsResponse | null>;
  ...
  getSeasonImages(showID: number, seasonNumber: number): Promise<TvSeasonImagesResponse | null>;
  getEpisodeImages(
    showID: number,
    seasonNumber: number,
  ...
  updateSeriesMetadata(series: Series, language: string): Promise<Series | undefined>;
  updateMovieMetadata(
    movie: Movie,
