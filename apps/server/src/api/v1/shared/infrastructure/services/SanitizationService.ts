...
function sanitizeFilePath(filePath: string, allowedBasePaths?: string[]): string {
  ...
  for (const pattern of dangerousPatterns) {
    if (pattern.test(decodedPath)) {
      throw new Error('Invalid file path: contains dangerous patterns (path traversal attempt)');
    }
  }
...
function sanitizeVideoPath(
  ...
  return sanitizeFilePathWithExtension(videoPath, videoExtensions, allowedBasePaths, shouldExist);
}
...
function sanitizeAudioPath(
  ...
  return sanitizeFilePathWithExtension(audioPath, audioExtensions, allowedBasePaths, shouldExist);
}
...
function sanitizeImagePath(
  ...
  return sanitizeFilePathWithExtension(imagePath, imageExtensions, allowedBasePaths, shouldExist);
}
...
function sanitizeMultipleFilePaths(
  ...
  return filePaths.map((filePath) => {
    if (allowedExtensions) {
      return sanitizeFilePathWithExtension(filePath, allowedExtensions, allowedBasePaths);
    }
