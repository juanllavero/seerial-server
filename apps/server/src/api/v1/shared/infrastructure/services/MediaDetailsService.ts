...
function buildLyricLookupCandidates(
	songBaseName: string,
	songTitle: string,
	trackNumber: number,
) {
...
function resolveLyricCandidateMatch(
	fileName: string,
	lyricCandidates: string[],
) {
	const lyricBaseName = normalizeForComparison(
...
export async function findLyricsForSong(songId: string): Promise<LyricsLine[]> {
	...
	try {
		...
		const lyricCandidates = buildLyricLookupCandidates(
			songBaseName,
			song.title,
			song.trackNumber,
		);
		...
		const matchLang = (f: string) =>
			resolveLyricCandidateMatch(f, lyricCandidates)?.language;
		const isTranslation = (lang: string | undefined) =>
			lang !== undefined && lang !== "original" && lang !== "pronunciation";

		const ttmlOriginal = filesInDir
			.filter(isTtml)
			.find((f) => matchLang(f) === "original");
		...
		if (ttmlOriginal) {
			...
			const pronunciationFile = lrcFiles.find(
				(f) => matchLang(f) === "pronunciation",
			);
			const [originalContent, pronunciationContent, translationContent] =
				await Promise.all([
...
export async function findMusicExtras(collectionId: string) {
	...
	const promises = Array.from(rootFolders).map((folder) =>
		findExtrasInFolder(folder),
	);
