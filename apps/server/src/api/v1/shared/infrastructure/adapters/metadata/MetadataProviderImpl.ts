...
export class MetadataProviderImpl implements MetadataProviderPort {
	 ...
	 */
	public async updateSeriesMetadata(
		series: Series,
		language: string,
	 ...
	 */
	public async updateVideoMetadataForMovie(
		video: Video,
		movie: Movie,
	): Promise<void> {
		try {
	...
	private ensureMovieImageDirectories(
		movieId: string,
		collectionId?: string,
	): void {
		const dirs = [
		...
		if (collectionId) {
			dirs.push(
				fileSystemService.getExternalPath(
					`resources/img/posters/${collectionId}`,
				),
			);
		}
