...
export async function getAudioInfo(
	...
	try {
		...
		const codec =
			streams.find((s) => s.codec_type === "audio")?.codec_name || "unknown";
		...
		const discNumber = tags.disc
			? Number.parseInt(tags.disc.split("/")[0], 10)
			: 0;
		const trackNumber =
			Number.parseInt((tags.track || tags.TRACK || "0").split("/")[0], 10) || 0;
		const composers =
