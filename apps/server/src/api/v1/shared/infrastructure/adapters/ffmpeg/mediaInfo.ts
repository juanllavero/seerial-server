	...
	try {
		...
		const mediaInfo: MediaInfo = {
			...
			bitrate: format.bit_rate
				? `${(format.bit_rate / 10 ** 3).toFixed(2)} kbps`
				: "0",
		...
		for (const stream of streams) {
			...
			if (codecType === "video") {
				videoTracks.push(
					processVideoData(stream as Parameters<typeof processVideoData>[0]),
				);
			} else if (codecType === "audio") {
				audioTracks.push(
					processAudioData(stream as Parameters<typeof processAudioData>[0]),
				);
			} else if (codecType === "subtitle") {
				subtitleTracks.push(
					processSubtitleData(
						stream as Parameters<typeof processSubtitleData>[0],
					),
				);
			}
		}
