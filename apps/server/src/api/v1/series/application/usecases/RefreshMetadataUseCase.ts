...
export class RefreshMetadataUseCase {
  ...
  async execute(seriesId: string): Promise<void> {
    ...
    try {
      ...
      await this.refreshSeasonsAndEpisodes(series.id, series.themdbId, library.language, series);
    } catch (error) {
  ...
  private async setSeriesAnalyzing(series: Series, analyzing: boolean): Promise<void> {
  ...
  private async refreshSeasonsAndEpisodes(
    ...
    for (const season of seasons) {
      ...
      for (const episode of episodes) {
        ...
        await this.metadataProvider.updateEpisodeMetadata(episode, video, series, episodeTMDb);
      }
    }
  }
}
