...
export class ScanSeriesUseCase {
	...
	async execute(library: Library, root: string): Promise<void> {
		...
		downloaderService.autoDownloadFirstAudioResult(
			`${show.name} main theme`,
			show.id,
		);
	 ...
	 */
	private async getOrCreateSeries(
		library: Library,
		root: string,
	): Promise<Series | null> {
	...
	private async hasFilesChanged(
		...
		const hasNewFiles = videoFiles.some(
			(file) => !(file in library.analyzedFiles),
		);
		...
		const missingFiles = analyzedFilesForThisFolder.filter(
			(file) => !videoFiles.includes(file),
		);
				...
				logger.debug(
					{
						...
						seasonInMetadata: seasonsMetadata.some(
	...
	async resolveEpisodeBySeasonEpisode(
		...
		if (!exists && show.episodeGroupId) {
			...
			return this.findEpisodeInGroup(
				resolvedGroup,
				seasonNumber,
				episodeNumber,
				seasonsMetadata,
		...
		return this.findEpisodeInSeasonIndex(
			seasonsIndex,
			seasonNumber,
			episodeNumber,
		);
	...
	getSeasonEpisodeByAbsoluteNumber(
		...
		const standardSeasons =
			this.getStandardSeasonsForAbsoluteMapping(seasonsMetadata);
