...
export class AudioProcessingServiceImpl implements AudioProcessingServicePort {
	...
	async ensureStemSeparationAvailable(): Promise<void> {
		...
		if (
			...
			throw new BadRequestException(
				messages.errors.server.audioSeparatorUnavailable,
			);
		}
	}
	...
	private startStemSeparationProcess(job: StemSeparationJob): void {
		...
		process.on("error", (error) => {
			audioProcessingLogger.error(
				{ error, songId: job.songId },
				"Stem separation process failed",
			);
			this.broadcastStemUpdate(job, {
				status: "error",
	...
	private handleStemProcessChunk(
		...
		for (const line of lines) {
			...
			if (severity === "error") {
				audioProcessingLogger.warn(
					{ songId: job.songId, stderr: line },
					"Stem separation stderr",
				);
			} else {
				audioProcessingLogger.info(
					{ songId: job.songId, stdout: line },
					"Stem separation stdout",
				);
			}
	...
	private broadcastStemUpdate(
		job: StemSeparationJob,
		update: StemSeparationUpdate,
	): void {
