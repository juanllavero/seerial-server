...
export class LibrariesRepositoryImpl
		...
		const collections = this.measureSync(
			...
			() =>
				libraryCollections.map(
					(libraryCollection) => libraryCollection.collection,
				),
		);
	...
	private async buildCollectionItems(
		...
		const items: Array<LibraryItem | null> = await Promise.all(
			collections.map(async (collection) => {
				...
				if (!shouldInclude) {
					this.logTiming(
						"buildCollectionItems.collection.total",
						collectionStartedAt,
						{
							...collectionContext,
							included: false,
				...
				const collectionWatched = this.measureSync(
					...
					() =>
						collectionWatchStates.length > 0
							? collectionWatchStates.every((item) => item)
							: false,
				);
	...
	private countCollectionItems(
		collection: CollectionModel,
		libraryId: string,
	): number {
		const moviesCount =
			collection.collectionMovies?.filter(
				(movie) => movie.movie.libraryId === libraryId,
			).length || 0;
		const seriesCount =
			collection.collectionSeries?.filter(
				(series) => series.series.libraryId === libraryId,
			).length || 0;
		const albumsCount =
			collection.collectionAlbums?.filter(
				(album) => album.album.libraryId === libraryId,
			).length || 0;
		...
		const collectionMovieIds = this.measureSync(
			...
			() =>
				new Set(
					collections.flatMap(
						(collection) =>
							collection.collectionMovies?.map((movie) => movie.movie.id) || [],
					),
				),
		);
		const collectionSeriesIds = this.measureSync(
			...
			() =>
				new Set(
					collections.flatMap(
						(collection) =>
							collection.collectionSeries?.map((series) => series.series.id) ||
							[],
					),
				),
		);
		const collectionAlbumIds = this.measureSync(
			...
			() =>
				new Set(
					collections.flatMap(
						(collection) =>
							collection.collectionAlbums?.map((album) => album.album.id) || [],
					),
				),
		);
		...
		switch (library.type) {
			case LibraryTypes.MOVIES:
				return await this.measureAsync(
					...
					async () => {
						const items = await this.buildMovieItems(
							library,
							collectionMovieIds,
							userId,
							watched,
			...
			case LibraryTypes.MUSIC:
				return await this.measureAsync(
					...
					async () => {
						const items = await this.buildAlbumItems(
							library,
							collectionAlbumIds,
						);
		...
		const moviesWithoutCollections = this.measureSync(
			...
			() =>
				(library.movies || []).filter(
					(movie) => !collectionMovieIds.has(movie.id),
				),
		);
	...
	private async buildSeriesItems(
		...
		const seriesWithoutCollections = this.measureSync(
			...
			() =>
				(library.series || []).filter(
					(series) => !collectionSeriesIds.has(series.id),
				),
		);
		...
		const filteredAlbums = this.measureSync(
			...
			() =>
				(library.albums || []).filter(
					(album) => !collectionAlbumIds.has(album.id),
				),
		);
	...
	private isMovieWatched(movie: MovieModel, userId: string): boolean {
		return (
			movie.watchLists?.some(
				(watchList) => watchList.userId === userId && watchList.watched,
			) ?? false
		);
	}
	...
	private shouldIncludeCollection(
		collectionWatchStates: boolean[],
		watched?: boolean,
	): boolean {
