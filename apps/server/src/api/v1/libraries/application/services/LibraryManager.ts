...
export const getCollectionImages = async (
	...
	const { posterPath, backgroundPath } =
		await findCollectionImagesInCollectionRoot(items);
	const resolvedPoster = posterPath ?? (coverSrc !== "" ? coverSrc : null);
	const resolvedBackground =
		backgroundPath ?? (backgroundSrc !== "" ? backgroundSrc : null);
