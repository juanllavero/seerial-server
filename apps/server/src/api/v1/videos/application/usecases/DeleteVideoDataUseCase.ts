...
export class DeleteVideoDataUseCase {
  ...
  async execute(videoId: string): Promise<void> {
    this.fileSystemService.deleteFolder(`resources/img/thumbnails/video/${videoId}`);
    this.fileSystemService.deleteFolder(`resources/img/thumbnails/chapters/${videoId}`);
  }
}
