...
export const useWebSocketStore = createWithEqualityFn<WebSocketState>((set, get) => ({
  ...
  setAnalyzing: (value, libraryId) => set({ analyzing: value, analyzingLibraryId: libraryId }),
            ...
            case MessageType.MUTATE_LIBRARY:
              ...
              mutate(
                ...
                (current: any[] | undefined) => {
                  ...
                  return updated.length === current.length ? [...current, message.body] : updated
                },
                { revalidate: false },
              )
  ...
  downloadVideo: async (elementId, url, libraryId, fileName) => {
    ...
    try {
      const response = await authenticatedFetch(`/api/downloadVideo`, 'POST', {
        url,
  ...
  downloadAudio: async (elementId, url, libraryId, fileName) => {
    ...
    try {
      const response = await authenticatedFetch(`/api/downloadMusic`, 'POST', {
        url,
