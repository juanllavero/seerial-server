...
function LyricsOptionsItem({
		...
		return (
			<div className="flex w-full items-center justify-between rounded-[1.8vh] px-[1.8vh] py-[1.4vh] text-left opacity-40">
				...
				<div className="flex items-center gap-[0.8vh]">
					{value && (
						<span className="text-[1.55vh] text-white/60">{value}</span>
					)}
	...
	return (
		...
		>
			<span
				className={`text-[1.85vh] font-medium ${focused ? "text-black" : "text-white/90"}`}
			>
				{label}
			</span>
			<div className="flex items-center gap-[0.8vh]">
				{value && (
					<span
						className={`text-[1.55vh] ${focused ? "text-black/70" : "text-white/60"}`}
					>
						{value}
					</span>
				)}
				...
				{showChevron && (
					<ChevronRight
						size={18}
						className={focused ? "text-black/70" : "text-white/55"}
					/>
				)}
			</div>
		</Button>
