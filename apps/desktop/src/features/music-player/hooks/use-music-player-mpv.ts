...
export function useMusicPlayerMpv({
    ...
    const { karaokeStemUrls, isKaraokePreparing, isKaraokeReady, shouldShowKaraokeButton, resetKaraokeAvailability } =
    ...
    const getActivePlaybackStatus = useCallback(async (): Promise<PlaybackStatus> => {
        if (playbackBackend === 'karaoke') {
            ...
            return { position: status.position, duration: status.duration, eofReached: status.eofReached };
        }
    ...
    const activateKaraokePlayback = useCallback(
        ...
        [getActivePlaybackPosition, isPlaying, karaokeStemUrls, playbackBackend, setIsLoading, setIsPlaying],
    );
