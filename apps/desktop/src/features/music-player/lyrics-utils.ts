...
function buildEnhancedWordLines(
  ...
  if (showPronunciation && words.pronunciation?.length) {
    ...
    out.push(
      buildAlignedLine(origText, true, origSegments, trackIndexByTimeKey, alignmentTrackWidths),
    );
    out.push(
      buildAlignedLine(pronText, true, pronSegments, trackIndexByTimeKey, alignmentTrackWidths),
    );
  } else {
 ...
 */
function splitTranslation(raw: string): {
  text: string;
...
function buildDisplayLines(line: LyricsLine, showPronunciation: boolean): LyricDisplayLine[] {
  ...
  for (const line of lyrics) {
    if (line.isBlank) {
      groups.push({
        time: line.startTime,
        agent: line.agent,
        lines: [{ text: '♪', isEnhanced: false, segments: [] }],
      });
      continue;
    }

    const displayLines = buildDisplayLines(line, showPronunciation);
    const backgroundVocals = line.words?.backgroundVocals?.length
