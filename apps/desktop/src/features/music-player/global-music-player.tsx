...
function useKaraokeAvailability(
	...
	const [karaokeStemUrls, setKaraokeStemUrls] =
		useState<KaraokeStemUrls | null>(null);
...
function useSongLyricsPanel(
	...
	const { data: lyrics = [], isLoading: isLyricsLoading } = useGetSongLyrics<
		LyricsLine[]
	>(songId ?? "", {
...
function useLyricsDisplayOptions(
	...
	const [showPronunciation, setShowPronunciation] = useState<boolean | null>(
		null,
	);
	...
	const hasPronunciation = useMemo(
		() =>
			lyrics.some(
				(l) =>
					!l.isBlank &&
					(!!l.words?.pronunciation?.length || !!l.plainText?.pronunciation),
			),
		[lyrics],
	);
...
function GlobalMusicPlayer() {
	...
	const [playbackBackend, setPlaybackBackend] =
		useState<PlaybackBackend>("mpv");
	...
	const getActivePlaybackStatus =
		useCallback(async (): Promise<PlaybackStatus> => {
	...
	const hideExpandedPlayer = useCallback(() => {
		...
		window.setTimeout(() => {
			const { isShown: stillShown, currentSong: stillCurrentSong } =
