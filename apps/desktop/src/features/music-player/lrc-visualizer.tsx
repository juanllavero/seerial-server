...
interface GroupRenderState {
  ...
  segActivationAudioTime: number | null;
  segActivationWallTime: number | null;
}
...
const EnhancedSegment = memo(function EnhancedSegment({
  ...
  useEffect(() => {
    ...
    const totalDurationSec = segment.endTime ? segment.endTime - segment.startTime : 0.3;
...
function getGroupKey(group: LyricGroup): string {
  return `${group.time}-${group.lines.map((line) => line.text).join('-')}`;
}

function getGroupRenderState(
  group: LyricGroup,
  index: number,
  currentLineIndex: number,
  upcomingOffsetY: number,
  ...
  const { containerClass, textClass, groupClass, isCurrentLine, style } = getLyricsLineState(
    index,
    currentLineIndex,
    upcomingOffsetY,
  );
  ...
  return {
    ...
    isPastLine,
    isV2,
    alignClass: isV2 ? 'pr-[2dvw] text-right' : 'pl-[2dvw] text-left',
    itemsClass: isV2 ? 'items-end' : 'items-start',
    containerAlignClass: isV2 ? 'ml-auto' : '',
  ...
  return (
    <>
      ...
      {!!group.translationText && (
        <span
          key="translation"
          className={`bg-transparent px-[1vh] leading-[1.3] whitespace-normal wrap-break-word transition-all duration-300 ease-out ${state.textClass} ${getLineSizeClass(2)}`}
        >
          {group.translationText}
        </span>
      )}
      {!!group.translationBackgroundVocals && (
        <span
          ...
          className={`bg-transparent px-[1vh] leading-normal whitespace-normal wrap-break-word opacity-75 transition-all duration-300 ease-out ${state.textClass} ${getLineSizeClass(2)}`}
        >
          {group.translationBackgroundVocals}
        </span>
      )}
...
function renderLyricGroupItem(
  ...
  const state = getGroupRenderState(
    group,
    index,
    currentLineIndex,
    upcomingOffsetY,
  ...
  return (
    <div
      ...
      className={`${state.alignClass} transition-[max-height,opacity,transform,filter,margin] duration-500 ease-out ${state.containerClass}`}
      style={state.style}
    >
      <div
...
function LRCVisualizer({
  ...
  const lineActivationRef = useRef<{
    audioTime: number;
    wallTime: number;
  } | null>(null);
      ...
      <div className="relative min-h-0 w-full flex-1 overflow-hidden" onWheel={handleWheel}>
        <div className="absolute inset-0 overflow-hidden px-[2.2vh]">
          {lyricGroups.map((group, index) =>
            renderLyricGroupItem(
              group,
              index,
              currentLineIndex,
              upcomingLineOffsets[index] ?? 0,
              lineActivationAudioTime,
              lineActivationWallTime,
            ),
          )}
