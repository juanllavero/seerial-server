...
function buildUpcomingLineOffsets(
	...
	for (let index = anchorIndex + 1; index < lyricGroups.length; index += 1) {
		...
		const weightedGap =
			UPCOMING_LINE_OFFSET_VH * ((previousWeight + currentWeight) / 2);
...
function getLyricsLineState(
	...
	let offsetY =
		relativeIndex > 0
			? upcomingOffsetY
			: relativeIndex * UPCOMING_LINE_OFFSET_VH;
	let opacity = 0;
	let scale = 0.96;
	let blurPx = 0;
...
const EnhancedSegment = memo(function EnhancedSegment({
	...
	useEffect(() => {
		...
		if (
			!isCurrentLine ||
			lineActivationAudioTime === null ||
			lineActivationWallTime === null
		) {
			el.style.transition = "none";
			el.style.width = "0%";
			return;
	...
	return (
		...
		>
			...
			<span className={isCurrentLine ? "text-white/30!" : "text-white/20"}>
				{segment.text}
			</span>
...
function LRCVisualizer({
	...
	if (prevLineIndexRef.current !== currentLineIndex || isSeeked) {
		...
		lineActivationRef.current =
			currentLineIndex >= 0
				? { audioTime: currentTime, wallTime: Date.now() }
				: null;
	}
	...
	if (lyricGroups.length === 0) {
		return (
			<FlexBox
				...
				className="px-[2vh]"
			>
				<span className="text-[2.2vh] font-semibold text-neutral-200">
					{t("lyricsNotFound")}
				</span>
			</FlexBox>
		);
			...
			>
				<div className="absolute inset-0 overflow-hidden px-[2.2vh]">
					{lyricGroups.map((group, index) => {
						...
						const segActivationAudioTime = isCurrentLine
							? lineActivationAudioTime
							: null;
						const segActivationWallTime = isCurrentLine
							? lineActivationWallTime
							: null;
						...
						return (
							...
							>
								<div
									className={`flex max-w-[56dvw] flex-col ${itemsClass} ${groupClass} ${isV2 ? "ml-auto" : ""}`}
								>
									{group.lines[0]?.alignmentTrackWidths &&
									group.lines[1]?.alignmentTrackWidths
										? [
