...
function QueueMenu({ isOpen, onClose }: QueueMenuProps) {
	...
	function getQueueSongLabel(
		song: Song,
		shouldIncludeDiscPrefix: boolean,
	): string {
		const discPrefix = shouldIncludeDiscPrefix
			? `${getSafeDiscNumber(song)}.`
			: "";
	...
	const shouldIncludeDiscPrefixInQueue = useMemo(() => {
		const queueDiscNumbers = new Set(
			songQueue.map((song) => getSafeDiscNumber(song)),
		);
			...
			>
				...
				>
					<div className="flex items-center justify-between px-[2.4vh] py-[2vh]">
						<Tertiary className="text-[2.1vh] font-semibold text-white">
							{t("upNext")}
						</Tertiary>
