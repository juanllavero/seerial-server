...
import { EditIcon, Film, MoreVertical, Music, SearchIcon, Trash2, TvMinimal } from 'lucide-react';
  ...
  const librariesItems = libraries
    ? [
        ...libraries.map((library) => ({
          id: library.id,
          name: library.name,
          logo: library.type === 'Shows' ? TvMinimal : library.type === 'Movies' ? Film : Music,
          action: () => {
                  ...
                  <SidebarMenuButton asChild>
                    <a
                      ...
                      className={`flex items-center gap-2 ${
                        activeItem && activeItem.id === item.id ? 'bg-accent' : ''
                      }`}
                      onClick={(e) => {
                        e.preventDefault();
                        setActiveItem(item);
