...
const GradientBackground: React.FC<GradientBackgroundProps> = ({ imageUrl }) => {
  ...
  useEffect(() => {
    ...
    const initPixi = async () => {
      ...
      try {
        ...
        try {
          ...
          if (loaded && loaded.source) texture = loaded;
        } catch (e) {
          console.warn("PixiJS no pudo parsear la URL, usando método nativo fallback...");
        }
        ...
        // --- Black Zone Fix ---
        const scaleMultipliers: number[] = [2.0, 1.25, 0.80, 0.50, 0.25];
    ...
    return () => {
      ...
      try {
        if (app && app.stage) app.destroy({ removeView: true }, { children: true });
      } catch (e) {
        console.warn("Pixi destroy aborted due to error:", e);
      }
    };
  }, [resolvedUrl]); 
