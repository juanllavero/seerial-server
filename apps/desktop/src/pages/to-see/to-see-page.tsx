...
function ToSeePage() {
	...
	const moviesLibraries = libraries?.filter(
		(library) => library.type === LibraryTypes.MOVIES,
	);
	const seriesLibraries = libraries?.filter(
		(library) => library.type === LibraryTypes.SHOWS,
	);
	...
	return (
		<ToSee
			moviesLibraries={moviesLibraries ?? []}
