...
function applySettingToMpv(
	key: keyof PlayerSettings,
	value: PlayerSettings[keyof PlayerSettings],
) {
	switch (key) {
		case "videoQuality":
			invoke("set_video_quality", {
				quality: value as PlayerVideoQuality,
			}).catch(console.error);
			break;
		case "zoom":
			...
			break;
		case "audioDelay":
			invoke("set_audio_delay", { delay: (value as number) / 1000 }).catch(
				console.error,
			);
			break;
		case "subtitleDelay":
			invoke("set_subtitle_delay", { delay: (value as number) / 1000 }).catch(
				console.error,
			);
			break;
		case "subtitleSize":
			invoke("set_subtitle_font_size", {
				size: SUBTITLE_SIZE_MAP[value as SubtitleSize],
			}).catch(console.error);
			break;
		case "subtitleColor":
			invoke("set_subtitle_color", {
				color: hexToMpvColor(value as string),
			}).catch(console.error);
			break;
		case "subtitleBorderSize":
			invoke("set_subtitle_border_size", { size: value as number }).catch(
				console.error,
			);
			break;
		case "subtitleShadowOffset":
			invoke("set_subtitle_shadow_offset", { offset: value as number }).catch(
				console.error,
			);
			break;
		case "subtitlePosition":
			invoke("set_subtitle_position_preset", {
