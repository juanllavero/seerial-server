...
function TracksSelectors({
	...
	const formatAudioTrackLabel = (track: AudioTrack) => {
		return [
			track.language === "" ? track.languageTag : track.language,
			track.displayTitle,
		]
			.filter(Boolean)
			.join(" ")
	...
	const handleSubtitleTrackChange = async (track: SubtitleTrack) => {
		...
		setTracks((currentTracks) => ({
			...currentTracks,
			subtitleTracks: updateSelectedTrack(
				currentTracks.subtitleTracks,
				track.id,
			),
		}));
		closePanel();
		await invoke("set_subtitle_track", { trackId: track.id }).catch(
			console.error,
		);
	};
	...
	const panelTracks =
		openPanel === "audio" ? tracks.audioTracks : subtitleOptions;
