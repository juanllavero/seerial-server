...
const VIDEO_QUALITIES: PlayerVideoQuality[] = [
	"low",
	"normal",
	"high",
	"ultra",
...
const SUBTITLE_SIZES: SubtitleSize[] = [
	"tiny",
	"small",
	"normal",
	"big",
...
function SettingRow({
	...
	return (
		<NavigationButton
			customKey={focusKey}
			disabled={disabled}
			className="mb-1 h-auto w-full justify-between rounded-xl! border border-white/5 bg-white/5 px-4 py-3 text-left text-sm transition-colors hover:text-black disabled:opacity-40"
		...
		>
			<div className="flex w-full items-center justify-between gap-3">
				...
				<div className="flex items-center gap-2">
					{colorSwatch && (
						<div
							className="size-4 border border-white/30"
...
function Settings({ onPanelChange, settings, updateSetting }: SettingsProps) {
	...
	const currentColor =
		SUBTITLE_COLORS.find((c) => c.hex === settings.subtitleColor) ??
		SUBTITLE_COLORS[0];
									...
									<SettingRow
										...
										onArrowPress={handleArrow("audioDelay", (dir) =>
											updateSetting(
												"audioDelay",
												settings.audioDelay +
													(dir === "right" ? DELAY_STEP : -DELAY_STEP),
											),
										)}
									/>
