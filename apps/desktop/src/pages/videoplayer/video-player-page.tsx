...
function VideoPlayerPage() {
	...
	const { data: video, isLoading: loadingVideo } = useGetVideo<Video>(
		videoId ?? "",
		{
	...
	const [showInitialLoadingBackdrop, setShowInitialLoadingBackdrop] =
		useState(true);
	const [isErrorDialogOpen, setIsErrorDialogOpen] = useState(false);
	const [errorMode, setErrorMode] = useState<PlayerErrorMode>("load");
	const [isRecoveringPlaybackError, setIsRecoveringPlaybackError] =
		useState(false);
	...
	const getInitialPlaybackPosition = useCallback(
		(videoToPlay: Video) => {
			const watchList = videoToPlay.watchLists?.find(
	...
	const pollPlaybackHealth = useCallback(async () => {
		...
		try {
			const playbackStatus = await invoke<PlaybackStatus>(
				"get_playback_status",
			);
	...
	const handleRetry = useCallback(async () => {
		...
		const retryPosition =
			errorMode === "playback"
				...
				: getInitialPlaybackPosition(video);
	...
	const shouldShowLoadingOverlay =
		!isErrorDialogOpen &&
		(isPlayerLoading || isPlaybackBuffering || !videoLoaded);
	const shouldUseBlackLoadingBackdrop =
		!videoLoaded || showInitialLoadingBackdrop;
