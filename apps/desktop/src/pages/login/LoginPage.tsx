...
function LoginPage() {
  const { user, server, setSelectedServer } = useServerStore(
    (state) => ({
      ...
      <div className="w-full max-w-2xl px-8">
        ...
        <div className="text-center">
          ...
          <div className="space-y-8">
            ...
            >
              {isLoading ? (
                <div className="h-7 w-7 animate-spin rounded-full border-3 border-stone-500 border-t-transparent" />
              ) : (
                'Conectar'
              )}
            </button>
          ...
          <div className="mx-auto mt-12 max-w-xl text-sm leading-relaxed text-stone-100">
            Introduce la dirección IP y puerto de tu servidor o la URL completa para conectarte
          </div>
        </div>
      </div>
    </div>
