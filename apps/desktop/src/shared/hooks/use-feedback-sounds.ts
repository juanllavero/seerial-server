...
export function useFeedbackSounds() {
	const feedbackSoundsEnabled = useSettingsStore(
		(state) => state.settings.feedbackSounds,
	);
	const movementAudioRef = useRef<HTMLAudioElement | null>(null);
	const interactionAudioRef = useRef<HTMLAudioElement | null>(null);

	useEffect(() => {
		movementAudioRef.current = createAudio(
			`${import.meta.env.BASE_URL}audio/movement.wav`,
		);
		interactionAudioRef.current = createAudio(
			`${import.meta.env.BASE_URL}audio/interaction.wav`,
		);

		return () => {
			for (const audio of [
				movementAudioRef.current,
