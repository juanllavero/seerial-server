import { BasicServer, BasicUser } from '@seerial/domain';
...
export const useServerStore = createWithEqualityFn<ServerState>((set, get) => ({
  ...
  servers: localStorage.getItem('servers') ? JSON.parse(localStorage.getItem('servers')!) : [],
  serverUrl: localStorage.getItem('serverUrl') || '',
  server: localStorage.getItem('server') ? JSON.parse(localStorage.getItem('server')!) : null,
  ...
  setApiKey: async (apiKey) => {
    ...
    try {
      const response = await authenticatedFetch(`${serverUrl}/api/api-key`, 'POST', { apiKey });
